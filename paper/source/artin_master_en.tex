...
universality in the $\sigma$-direction and placing $S_{3}$ in complete
agreement with Paper~2's thirteen-$L$-function comparison table
...
\smallskip
\noindent Four numerical results (\#121--\#124) support the
...
\begin{enumerate}
  \item \textbf{Icosahedral representations.}
    ...
    These require significantly more computational resources for
...
This paper extends the $\xi$-bundle framework in three directions.
...
$2.0000\pm0.0000$ ($R^{2}=1.000000$), placing $S_{3}$ in full agreement
with Paper~2's universal thirteen-$L$-function comparison table
(degrees~1--6, all slopes~$\approx 2.0$).  The two directions are
complementary: the $\sigma$-direction slope~$=+2$ is universal across
the Selberg class (consequence of $c_{1}=0$ for on-critical zeros),
while the t-direction slope~$\approx{-1}$ reflects individual analytic
structure specific to each $L$-function.  Third, we extend the
off-critical curvature discriminant
...
\caption{Summary of the four numerical results in this paper.}
\label{tab:summary} \\
\toprule
