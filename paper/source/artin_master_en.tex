...
We extend the $\xi$-bundle framework (Kim~2026a,b) to non-abelian
Artin $L$-functions and establish the off-critical universality of the
curvature discriminant $c_{1}$ across multiple $L$-function families.

For the standard representations of $S_{3}$ ($\mathrm{GL}(2)$,
$N=23$) and $S_{4}$ ($\mathrm{GL}(3)$, $N=283$), we verify all four
$\xi$-bundle properties: functional-equation consistency, zero
enumeration, $\kap\delta^{2}$-scaling with slopes $-0.994\pm0.001$
and $-0.993\pm0.003$ respectively (t-direction), and monodromy $\pm\pi$.
These constitute the first numerical verification of the $\xi$-bundle
framework on non-abelian Galois representations.

Additionally, a $\sigma$-direction reanalysis of $S_{3}$ via PARI/GP
\texttt{lfundiv} (Result~\#207) yields slope $2.0000\pm0.0000$
($R^{2}=1.000000$, five zeros, FE${}=-197$), confirming $c_{1}=0$
universality in the $\sigma$-direction.
A new measurement of the $S_{5}$ Artin $L$-function (degree~$4$,
$|S_{5}|=120$; Result~\#209) yields slope $1.9999\pm0.0003$
($R^{2}=1.000000$, five zeros, monodromy $5/5=2.0000\pi$,
$\sigma$-uniqueness $5/5$ PASS), confirming $\kap\delta^{2}$
universality for a second Artin family and directly rebutting chain-bias
concerns for degree~$4$.  Both results place the Artin family in complete
agreement with Paper~2's fourteen-$L$-function comparison table
(degrees~1--6, all slopes~$\approx 2.0$).  The two-directional structure
--- t-direction slope~$\approx{-1}$ (individual analytic structure)
and $\sigma$-direction slope~$=2.0$ (universal $c_{1}=0$ law) ---
provides a richer characterisation of the $\xi$-bundle geometry for
...
framework's extension to non-abelian arithmetic, the universality
of the curvature discriminant, and the Artin family universality
across two Galois groups ($S_{3}$, $S_{5}$).
\end{tcolorbox}

\vspace{0.5em}
\noindent\textbf{Keywords:} $\xi$-bundle, Artin $L$-function,
non-abelian Galois representation, $S_{3}$, $S_{4}$,
...
The P3 measurements in \cref{sec:s3-results} follow the \emph{t-direction}
protocol: $\delta$ is a small offset along the imaginary axis from a
zero at $t_{0}$, and $\log\kap$ vs $\log\delta^{2}$ is expected to
have slope~$-1$.  Paper~2's thirteen-$L$-function comparison table uses
the complementary \emph{$\sigma$-direction} protocol: $\delta$ is an
offset along the real axis from $\sigma_{0}=\tfrac{1}{2}$, for which
the expected slope is~$+2$.  To resolve an apparent discrepancy
between the two tables and to complete the picture, we applied the
$\sigma$-direction protocol to $S_{3}$ using PARI/GP's
\texttt{lfundiv} (Result~\#207).

\paragraph{Setup.}
The Artin $L$-function $L(s,\rho)$ is realised as
$\mathrm{lfundiv}(\zeta_{K},\zeta)$ in PARI/GP at
realprecision~$57$.  Functional-equation accuracy:
$\texttt{lfuncheckfeq}=-197$ (rp~$57$) and $-393$ (rp~$100$).
From 54~zeros in $t\in[0,60]$, five were selected with mutual
gap~${>}1.0$: $\gamma_{1}=5.1157$, $\gamma_{2}=7.1593$,
$\gamma_{3}=8.8814$, $\gamma_{4}=10.2820$, $\gamma_{5}=11.4300$.

\paragraph{Results.}
\Cref{tab:s3-sigma} presents the individual slopes;
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Zero $\gamma_{k}$} & \textbf{Slope} & $R^{2}$ & \textbf{Dev.} \\
\midrule
...
Monodromy: $\mono/\pi=2.0000$ for all five zeros (radius~$0.01$,
64~steps).  \textbf{PASS} on $\sigma$-direction P3 and P4.

\begin{table}[ht]
\centering
\caption{Two-directional $\kap\delta^{2}$ slopes for $S_{3}$.}
\label{tab:s3-twodir}
\begin{tabular}{@{}llccc@{}}
\toprule
...
The two measurements probe different aspects of the $\xi$-bundle geometry.

\begin{itemize}
  \item \textbf{$\sigma$-direction (slope~$=+2.0$)}: reflects the
    universal property $c_{1}=0$ for on-critical zeros
    ($\sigma_{0}=\tfrac{1}{2}$).  When $\delta$ is displaced in the
    $\sigma$-direction from an on-critical zero,
    $\kap\delta^{2}=1+O(\delta^{2})$ so the leading power is always
    $+2$, regardless of the specific $L$-function.  This universality
    is confirmed across all thirteen $L$-functions in Paper~2's
    comparison table (degrees~1--6, slopes~$\approx 2.0$);
    $S_{3}$ now joins this list in perfect agreement.
  \item \textbf{t-direction (slope~$\approx{-1}$)}: encodes the
    individual analytic structure of the $L$-function via the
    $Z''/Z'$ term.  For $S_{3}$ the slope is~$\approx{-1}$,
    matching the theoretical prediction and confirming P3.
\end{itemize}

The earlier apparent \emph{Artin anomaly}---$S_{3}$ appearing as
the sole outlier in slope comparisons---arose from comparing
t-direction and $\sigma$-direction measurements without
distinguishing the two protocols.  In the $\sigma$-direction,
$S_{3}$ is fully universal.

\begin{observation}[Two-directional $\kap\delta^{2}$ universality]
\label{obs:bidir}
For the $S_{3}$ Artin $L$-function ($\mathrm{GL}(2)$, $N=23$), the
$\kap\delta^{2}$ scaling yields slope~$\approx{-1}$ in the t-direction
and slope~$=2.0000\pm0.0000$ in the $\sigma$-direction.  The latter
places $S_{3}$ in complete agreement with the universal value observed
across degrees~1--6 in Paper~2, confirming that $c_{1}=0$ universality
holds for non-abelian Artin $L$-functions.
\end{observation}
...
(degree~$4$), constructed as $\mathrm{lfundiv}(\zeta_K,\zeta)$ in PARI/GP
...
FE${}=-393$ digits (\texttt{rp}$=100$), $-199$ at \texttt{rp}$=57$.

\paragraph{$\sigma$-direction $\kap\delta^{2}$ measurement.}
Using the same $\sigma$-direction protocol as for $S_{3}$
(\S\ref{sec:s3-sigma}), five zeros
($t\in[2.79,8.84]$, gap~$>1.0$) are selected from 49~zeros in
$t\in[0,35]$: $\gamma_{1}=2.7937$, $\gamma_{2}=4.0888$,
...
Monodromy: $\mono/\pi=2.0000$ for all five zeros.
$\sigma$-uniqueness: $5/5$ PASS (centre $\sigma=\tfrac{1}{2}$ is maximal).
Overall: \textbf{4/5 PASS} ($\bigstar\bigstar\bigstar$).
(SC1 off-critical FAIL is a computational artefact common to Artin
$\zeta_K/\zeta$ constructions, identical to \#207; critical-line
rel\_err$=0.00$.)
...
\caption{Comparison of $S_{3}$ and $S_{5}$ Artin $L$-functions
  ($\sigma$-direction measurements).}
\label{tab:s3-s5-compare}
\begin{tabular}{@{}lcccccl@{}}
\toprule
$L$-\textbf{function} & \textbf{Deg.} & $|G|$ & $N$
...
\begin{enumerate}
  \item \textbf{Slope universality across Galois complexity}: The
    $\kap\delta^{2}$ slope remains at $\approx 2.0$ as the Galois group
    order increases 20-fold ($|S_{3}|=6 \to |S_{5}|=120$) and the
    degree doubles ($2\to 4$).  This confirms that the universal slope
    is insensitive to the algebraic complexity of the underlying
    Galois representation.
  \item \textbf{$\sigma$-uniqueness contrast}: $S_{3}$ fails
    $\sigma$-uniqueness (B-01 boundary, $N=23$, curvature peak displaced
    from $\sigma=\tfrac{1}{2}$), while $S_{5}$ passes
    (conductor $N=2869$, but $\sigma_{\mathrm{crit}}=\tfrac{1}{2}$,
    so the curvature maximum lies on the critical line).  This contrast
    shows that $\sigma$-uniqueness depends on the conductor geometry
    and critical-line position, not on the Galois group structure.
\end{enumerate}
...
$\kap\delta^{2}$ slope equals $2.0\pm 0.001$ with $R^{2}=1.000000$.
The universality of the slope holds across a 20-fold increase in
Galois group order and a doubling of the Selberg class degree.
Combined with two sym$^{3}$ measurements at degree~$4$ (Results~\#203,
\#204), three independent GL(4) constructions now confirm the
slope~$2$ universality, directly rebutting any chain-bias concern.
\end{observation}
...
3 & \textbf{Artin $S_{5}$ (non-abelian, \#209)} & \textbf{4} & \textbf{$\sigma$-slope $=1.9999$~\checkmark{}; $\sigma$-uniq.\ PASS; chain-bias rebuttal} \\
...
line.  The consistency of the framework across this landscape supports
...
\begin{enumerate}
  \item \textbf{Icosahedral representations.}
    ...
    The present $S_{5}$ result (Result~\#209, degree~4) confirms the
    $\kap\delta^{2}$ universality for $|S_{5}|=120$, but $A_{5}$
    (degree-5 icosahedral) requires significantly more computational
    resources for coefficient computation.
  \item \textbf{Higher conductor.}
    Our Artin examples have $N=23$ and $N=283$.  For $N > 1000$, the
    AFE convergence slows and numerical stability may require
    $\text{dps} > 200$.
...
89~zeros) achieve $\kap\delta^{2}$ t-direction slopes within 1\% of
the theoretical value across multiple measurement groups, with
$R^{2} > 0.9999$.  Second, a $\sigma$-direction reanalysis of $S_{3}$
(Result~\#207, PARI/GP \texttt{lfundiv}) yields slope
$2.0000\pm0.0000$ ($R^{2}=1.000000$).  Third, a new measurement of the
$S_{5}$ Artin $L$-function (Result~\#209, degree~$4$, $|S_{5}|=120$,
conductor $N=2869$) yields slope $1.9999\pm0.0003$ ($R^{2}=1.000000$),
monodromy $5/5=2.0000\pi$, and $\sigma$-uniqueness $5/5$ PASS.
Results~\#207 and \#209 together place both Artin families in full
agreement with Paper~2's universal fourteen-$L$-function comparison
table (degrees~1--6, all slopes~$\approx 2.0$), and establish that
the slope~$2$ universality holds across a 20-fold increase in Galois
group order ($|S_{3}|=6 \to |S_{5}|=120$) and a degree doubling
($2 \to 4$).  Combined with two sym$^{3}$ measurements at degree~$4$,
three independent GL(4) constructions confirm the universality,
directly rebutting chain-bias concerns.  The two measurement directions
are complementary: the $\sigma$-direction slope~$=+2$ is universal
(consequence of $c_{1}=0$ for on-critical zeros), while the
t-direction slope~$\approx{-1}$ reflects individual analytic structure.
Fourth, we extend the off-critical curvature discriminant
$c_{1}\cdot\abs{\sigma_{0}-\tfrac{1}{2}} \to 1$ from a single
$L$-function (mod~5 DH) to three independent families (mod~5, 7, 11),
with $N=12$ off-critical zeros confirming the asymptotic law.
...
forms with and without Euler products, and zeros on and off the
critical line.  The universality of the four properties across this
landscape, confirmed in both the t-direction and the $\sigma$-direction
for non-abelian Artin $L$-functions, provides strong numerical evidence
that the $\xi$-bundle captures a fundamental geometric structure of
$L$-function zeros.
...
122 & Artin $S_{3}$ (t-dir.\ precision) & $\bigstar\!\bigstar\!\bigstar$ &
  t-direction slope $-0.993\pm0.001$ (AFE) &
...
209 & Artin $S_{5}$ ($\sigma$-direction) & $\bigstar\!\bigstar\!\bigstar$ &
  ...
  $\sigma$-uniq $5/5$ PASS; $|S_5|=120$; chain-bias rebuttal &
