...
\begin{enumerate}[label=(\roman*)]
  ...
  \item 반상관은 차수 1--6의 열두 개의 $L$-함수에서 검증되어
        상관의 \emph{부호}에 대한 차수-독립 보편성을 확립한다.
  \item GUE 랜덤 행렬 앙상블은 전체합에서
        ...
        산술 감쇠는 $d = 1$에서 $\delta \approx 0.04$에 불과(비유의)하고,
        GL(2)에서는 진정한 감쇠
        $\delta \approx 0.24$ (${\sim}\,2\sigma$, $d = 2$)를 보인다.
        이전에 보고된 보편적 감쇠 $0.38$은
        방법론적 인공물이었다 (\cref{rem:locality}).
...
\begin{enumerate}
  \item \textbf{$\delta(d)$의 수론적 기원.}\;
        산술 감쇠는 차수-의존적이다:
        $\delta \approx 0.04$ ($d = 1$), $\delta \approx 0.24$ ($d = 2$),
        $\delta \ge 0.39$ ($d \ge 3$, $T$-제한 상한).
        $\delta(d)$가 conductor, 감마 인자 복잡성($d$개 감마 이동),
        혹은 오일러 곱 깊이에 의해 제어되는가?
        관계식 $\delta \sim f(d, N)$은 진폭--간격 현상을
        Langlands 함수성과 연결할 것이다.
  \item \textbf{$d \ge 3$에서의 $T$ 확장.}\;
        \cref{tab:unified}의 차수 $\ge 3$ $L$-함수들은
        모두 $T \le 35$로 제한되어 트림이나 국소합 보정이 불가하다.
        $T \ge 100$으로의 확장(계산 가능 시)은
        $d \ge 3$의 큰 $\delta$ 값이 진정한 것인지
        짧은 $T$-범위의 추가 인공물인지 결정할 것이다.
