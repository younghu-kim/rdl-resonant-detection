...
(i)~\emph{보편적}: GL(1)부터 GL(5)까지 열한 개의 자기동형 $L$-함수에서
일관되게 음수이고,
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{차수-독립 보편성}
        ...
        $\rho(A, g) < 0$이 GL(1)부터 GL(5)까지 열한 개의 $L$-함수에서
        검증된다. 대상에는 Riemann $\zeta$, Dirichlet $L$-함수,
        타원곡선 $L$-함수, 그리고 $\mathrm{sym}^4$까지의 대칭 거듭제곱이 포함된다.
...
Langlands 계층의 차수 1--5에 걸친 열한 개의 $L$-함수에서
진폭--간격 반상관을 검증한다.
GL(1) 및 GL(2) $L$-함수의 경우 Cauchy 윤곽 진폭 $A_\La$이
자연스럽게 $g_{\mathrm{right}}$와 짝을 이루고,
GL(3)부터 GL(5)까지의 경우 영점-합 진폭 $A_L$이
$g_{\min}$과 짝을 이룬다
(이 짝짓기의 설명은 \cref{sec:dichotomy} 참조).

\begin{table}[h]
\centering
\caption{열한 개의 $L$-함수에 걸친 진폭--간격 Spearman 상관.
모든 상관이 음수($p < 10^{-4}$).}
...
열한 개의 $L$-함수 모두 $p < 10^{-4}$의 음의 진폭--간격 상관을 보인다.
상관 강도 $\rho \in [-0.63, -0.42]$는 차수($d = 1$--$5$),
...
\begin{enumerate}[label=(\roman*)]
  \item 무조건적 하한 $A \ge 2/\Delta_{\min}^2$
        ...
        미증명 가설을 필요로 하지 않는다.
  \item 반상관은 차수 1--5의 열한 개의 $L$-함수에서 검증되어
        차수-독립 보편성을 확립한다.
