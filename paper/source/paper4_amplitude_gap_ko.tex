...
산술 감쇠 $|\rho_{\GUE}| - |\rho_L| \approx 0.38 \pm 0.04$
자체도 차수-독립적이어서, GUE 기준선으로부터의 약화가 차수 특이적 효과가 아닌
산술적 $L$-함수의 보편적 특성임을 보여준다.
...
$N \in \{50,100,200,500,1000,2000\}$의 GUE$(N)$ 행렬에 대해 계산하고,
$g_{\min}^{\GUE}$와의 Pearson 및 Spearman 상관을 모두 보고한다
(중앙 $80\%$ 고유값, $N$당 $20$--$100$회 실현).
...
\begin{enumerate}
  \item \textbf{Spearman 안정성.}\;
        $\rho_{\mathrm{S}}(A,g_{\min}) = -0.857 \pm 0.003$으로
        $N = 50$--$2000$에서 상수이다 (모든 $N$에서 $p \approx 0$).
        진폭--간격 반상관은 행렬 크기에 무관한 GUE 내재적 성질이다.
  \item \textbf{Pearson 아티팩트.}\;
        Pearson $\rho_{\mathrm{P}}$는 $-0.12$에서 $-0.33$ 사이에서
        요동한다. $A$의 중꼬리 분포(작은 간격 근방의 $1/\Delta^2$ 지배)가
        선형성 가정을 위반하기 때문이다. 순위 기반인 Spearman은 이에 면역이다.
  \item \textbf{산술적 감쇠.}\;
        GUE의 Spearman 값 $|\rho_{\mathrm{S}}| = 0.86$은
        $L$-함수의 값 $0.42$--$0.58$(\cref{sec:univ})을 초과한다.
        산술 구조는 반상관을 \emph{생성}하는 것이 아니라 \emph{약화}시킨다.
\end{enumerate}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item 반상관은 차수 1--6의 열두 개의 $L$-함수에서 검증되어
        차수-독립 보편성을 확립한다.
  \item GUE 랜덤 행렬 앙상블은 Spearman 반상관
        $\rho_{\mathrm{S}} = -0.857 \pm 0.003$
        ($N = 50$--$2000$에서 상수)을 보이며, $L$-함수($0.42$--$0.58$)보다
        \emph{더 강하다}: 산술 구조는 상관을 생성하는 것이 아니라 감쇠시킨다.
...
\begin{enumerate}
  ...
  \item \textbf{산술 감쇠 메커니즘.}\;
        GUE 기준선 $|\rho_{\mathrm{S}}| = 0.86$이 모든 산술적 $L$-함수에서
        $|\rho_{\mathrm{S}}| \approx 0.47$로 감쇠하며, 이 차수-독립적
        손실 ${\sim}0.38$이 감마 인자에서 유래하는지 오일러 곱에서 유래하는지
        추적할 수 있는가?
