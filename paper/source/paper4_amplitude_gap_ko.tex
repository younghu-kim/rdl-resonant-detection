...
차수~1--4의 열한 개 $L$-함수에서
$\rho_{\mathrm{S}} = -0.894 \pm 0.014$ (범위~$0.043$)이며,
...
\begin{table}[h]
\centering
\caption{열한 개 $L$-함수(자기쌍대 7개, 비자기쌍대 4개, 차수 1--4)에서의
정준 메트릭 진폭--간격 상관. 모든 $p$-값은 $10^{-76}$ 미만.}
\label{tab:canonical}
\small
...
\midrule
\multicolumn{3}{l}{자기쌍대 평균 ($n = 7$)} & & $-0.896$ & & $\varepsilon = \pm 1$ \\
\multicolumn{3}{l}{비자기쌍대 평균 ($n = 4$)} & & $-0.887$ & & $\varepsilon \in S^1$ \\
\multicolumn{3}{l}{전체 평균 ($n = 11$)} & & $-0.894$ & & \\
\multicolumn{3}{l}{전체 표준편차} & & $0.014$ & & \\
...
평균 Spearman 상관은 자기쌍대 가족($\varepsilon = \pm 1$, GL(4) 2개 포함)에서 $-0.896$,
비자기쌍대 가족($\varepsilon \in S^1$)에서 $-0.887$로,
차이 $0.009$는 개별 $95\%$ 신뢰구간 범위 내에 있다.
따라서 상관은 근호수에 무관하며,
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item 반상관은 차수 1--6의 열두 개의 $L$-함수에서 검증되어
        ...
        차수 1--4의 열한 개 가족이 $\rho = -0.894 \pm 0.014$이다
        (\cref{tab:canonical}).
