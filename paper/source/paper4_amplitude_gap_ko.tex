...
차수~1--5의 열두 개 $L$-함수에서
$\rho_{\mathrm{S}} = -0.895 \pm 0.015$ (범위~$0.043$)이며,
...
\begin{table}[h]
\centering
\caption{열두 개 $L$-함수(자기쌍대 8개, 비자기쌍대 4개, 차수 1--5)에서의
정준 메트릭 진폭--간격 상관. 모든 $p$-값은 $10^{-70}$ 미만.}
\label{tab:canonical}
\small
...
\midrule
\multicolumn{3}{l}{자기쌍대 평균 ($n = 8$)} & & $-0.899$ & & $\varepsilon = \pm 1$ \\
\multicolumn{3}{l}{비자기쌍대 평균 ($n = 4$)} & & $-0.887$ & & $\varepsilon \in S^1$ \\
\multicolumn{3}{l}{전체 평균 ($n = 12$)} & & $-0.895$ & & \\
\multicolumn{3}{l}{전체 표준편차} & & $0.015$ & & \\
\multicolumn{3}{l}{전체 범위} & & $0.043$ & & \\
\bottomrule
\end{tabular}
\end{table}

\begin{observation}[차수별 추세]\label{obs:degree_trend}
$|\rho_{\mathrm{S}}|$가 차수에 따라 약간 강화되는 경향이 관찰된다:
차수~$1$--$3$ 평균 $= -0.891$, 차수~$4$ 평균 $= -0.907$,
차수~$5$ $= -0.913$.  전체 표준편차 $0.015$의 $1\sigma$ 범위 내이므로
통계적 변동일 가능성이 있으나, 향후 연구에서 조사할 가치가 있다.
특히 유일한 GL(5) 사례인 $\mathrm{Sym}^4(E_{11\mathrm{a}1})$가
데이터셋 내 가장 강한 상관을 보이므로, 추가 GL(5) $L$-함수로의
확인이 이 효과가 체계적인지 통계적 잡음인지 판별할 수 있을 것이다.
\end{observation}

\begin{observation}[근호수 독립성]\label{obs:root_number}
평균 Spearman 상관은 자기쌍대 가족($\varepsilon = \pm 1$, GL(4) 2개와 GL(5) 1개 포함)에서 $-0.899$,
비자기쌍대 가족($\varepsilon \in S^1$)에서 $-0.887$로,
차이 $0.012$는 개별 $95\%$ 신뢰구간 범위 내에 있다.
따라서 상관은 근호수에 무관하며,
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item 반상관은 차수 1--6의 열두 개의 $L$-함수에서 검증되어
        ...
        차수 1--5의 열두 개 가족이 $\rho = -0.895 \pm 0.015$이다
        (\cref{tab:canonical}).
