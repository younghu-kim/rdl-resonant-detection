...
$\chi\bmod 8$ (실수, $q{=}8$) & $405^{*\ddagger}$ & $\sigma{=}0.5$만 & $53\times^{\dagger}$ & 0.0000 & $266\times$ \\
\bottomrule
\end{tabular}
\end{center}
{\small ${}^*$~$t\in[10,200]$; $q=3,4,5$보다 넓은 범위 ($t\in[10,40]$). \quad
${}^{\dagger}$~직접 $\sigma$-비교: $E(0.5)/E(0.3)$,
$\delta{=}0.03$ ($q=3,4,5$의 근접/원격 방법과 다름). \quad
${}^{\ddagger}$~3개 지표 합계 (원시 2개 + $\chi\bmod 4$에서 유도 1개);
아래 지표별 분해 참조.}

\noindent
$q=3,4,5$ 3개 지표 38개 영점 전체에서 모노드로미 $\Delta\arg\Lambda = \pm\pi$ (편차 $< 10^{-4}$);
$\chi\bmod 7$ ($q=7$)에서는 136개 영점, $\chi\bmod 8$ ($q=8$)에서는
405개 영점 모두 편차 $0.000000\pm 0.000000$으로 동일 조건 충족 (로그 공간 계산).
위상 점프는 $\sigma = 1/2$에서만 발생 ($t\in[10,40]$, 11개 $\sigma$ 값 검사; $q=7$에서도 확인). 곡률 집중도 비율은 영점 인접 점($\pm 0.01$) 중앙값 $\kappa$ 대 일반 점(영점에서 $>1.0$ 거리) 중앙값.
...
\item \emph{$q{=}7$ 지표별 분해.}\;
\textcolor{ForestGreen}{[완료 2026-04-26]}
지표군 $(\mathbb{Z}/7\mathbb{Z})^\times$의 $\varphi(7)-1 = 5$개 비자명 지표에 대해
($t\in[10,70]$, $\delta=0.03$, $\varepsilon_{\mathrm{mono}}=0.005$, \texttt{dps}$=50$)
개별 4성질 검증을 수행했다.
...
\toprule
\textbf{지표} & \textbf{패리티} & \textbf{영점수}
  & $\kappa\delta^2$ \textbf{중앙}
  & \textbf{모노 편차} & \textbf{감지율}
  & $E(0.5)/E(0.3)$ \\
\midrule
$\chi_1$ ($j{=}1$, 위수~6) & 홀  & 34 & 1.0032 & 0.0000 & 100\% & $104.4\times$ \\
$\chi_2$ ($j{=}2$, 위수~3) & 짝 & 35 & 1.0029 & 0.0000 & 100\% & $30.1\times$ \\
$\chi_3$ ($j{=}3$, 르장드르) & 홀  & $34^*$ & 1.0025 & 0.0000$^*$ & 100\% & $166.9\times$ \\
$\chi_4$ ($j{=}4$, 위수~3) & 짝 & 35 & 1.0032 & 0.0000 & 100\% & $337.8\times$ \\
$\chi_5$ ($j{=}5$, 위수~6) & 홀  & 35 & 1.0033 & 0.0000 & 100\% & $557.3\times$ \\
\midrule
\multicolumn{2}{l}{\textbf{합계 (5개 지표)}} & \textbf{173} & 1.003 & 0.0000 & 100\% & $239\times$ \\
\bottomrule
\end{tabular}
\end{center}
{\small ${}^*$~$\chi_3$ (르장드르 기호 $(\frac{\cdot}{7})$, 실수 이차 지표):
가짜 영점 3건 제거 (음수 $t$ 탈출 1건, 밀집 간격 아티팩트 2건).
필터 후 진짜 영점 34개에서 $34/34 = 100\%$ 모노드로미 통과.
\texttt{findroot} 구현 아티팩트이지 프레임워크 한계가 아니다.}

\smallskip\noindent
6-원소 지표군에 대한 최초의 지표별 검증이다.
5개 지표 — 양쪽 패리티(홀/짝), 세 위수(2, 3, 6), 실수/복소 — 가
모두 4성질을 통과한다: $\kappa\delta^2$은 이론값~1에서 0.3\% 이내,
감지율 100\%. 에너지 비율 $E(0.5)/E(0.3)$은 $30\times$~$557\times$으로
지표마다 달라지는데, 이는 도체-의존 $A(t_0)$ 보정
및 가우스 합 기여와 일관된다.
\item \emph{$q{=}8$ 지표별 분해 (첫 합성수 도체).}\;
\textcolor{ForestGreen}{[완료 2026-04-26]}
지표군 $(\mathbb{Z}/8\mathbb{Z})^\times = \{1,3,5,7\} \cong \mathbb{Z}/2\times\mathbb{Z}/2$
(클라인 4-군)은 비자명 지표 3개를 가지며, 모두 위수~2 (실수).
원시 지표 2개(도체~8)와 $\chi\bmod 4$에서 유도된 지표 1개로 구성된다.
($t\in[10,200]$, $\delta=0.03$, $\varepsilon_{\mathrm{mono}}=0.005$, \texttt{dps}$=80$)
...
\textbf{지표} & \textbf{패리티} & \textbf{영점수}
  ...
  & $E(0.5)/E(0.3)$ \\
\midrule
$\chi_B$ (원시, 도체~8) & 홀  & 142 & 1.003 & 0.0000 & 100\% & $53\times$ \\
$\chi_C$ (원시, 도체~8) & 짝 & 142 & 1.003 & 0.0000 & 100\% & $20\times$ \\
$\chi_A$ (유도 $\leftarrow$mod~4)${}^\dagger$ & 홀  & $121^*$ & 1.023 & 0.0000$^*$ & 100\% & $725\times$ \\
\midrule
\multicolumn{2}{l}{\textbf{합계 (3개 지표)}} & \textbf{405} & 1.003 & 0.0000 & 100\% & $266\times$ \\
\bottomrule
\end{tabular}
\end{center}
{\small ${}^\dagger$\,$\chi_A$는 $\chi\bmod 4$에서 유도됨.
$8 = 2^3$이고 $f(\chi_A) = 4$이므로 $p=2$의 오일러 인자는
추가 영점을 만들지 않아 $L(s,\chi_A\bmod 8) = L(s,\chi\bmod 4)$이다.\\
${}^*$\,영점 탐색기가 139개 후보를 반환하나 121개만 $\chi\bmod 4$
영점과 일치 (87.1\%).  18개 추가 후보($t=13.60, 22.66$ 등)는
$\mathrm{mono}/\pi \approx 0.001$로 스캔 아티팩트이다
(위 $\chi_3\bmod 7$과 동일 패턴). 제거 후 $121/121 = 100\%$ 통과.}

\smallskip\noindent
첫 합성수 도체 검증이다.
클라인 4-군 구조(전부 위수~2, 전부 실수)는 순환군 $q=5,7$에 대한
상보적 검증을 제공한다.
원시 지표 2개는 $\kappa\delta^2$이 1에서 0.3\% 이내로 4성질을 통과한다.
유도 지표 $\chi_A$는 프레임워크가 도체-모듈러스 분리를 올바르게 처리함을
확인한다: $\xi$-다발은 오일러 곱 구조의 예측대로 원시 $L$-함수의
영점만을 정확히 감지한다.
