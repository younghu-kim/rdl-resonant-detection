...
\noindent
${}^\ddagger$\,도체 8의 원시 지표 2개 ($\chi_8$ 짝, $\chi_4$ 홀) + 유도 지표 1개 (조건부 PASS).
...
\phantomsection\label{tab:conjugate_eratio}
\begin{center}
\small
\begin{tabular}{lcccl}
\toprule
...
\begin{remark}[켤레 지표의 $E$-비율 비동치]\label{rem:conjugate_nonequiv}
표~\ref{tab:conjugate_eratio}는 주목할 만한 현상을 보여 준다:
켤레 지표 $\chi$와 $\bar\chi$는 동일한 $|L(1,\chi)|$, 동일한 영점 수,
동일한 $\kappa\delta^2$ 통계를 공유하지만,
$E$-비율은 $1.12\times$에서 $144\times$에 이르는 인수만큼 차이가 난다.

$q = 3, 4, 5, 7, 8, 11$의 원시 지표 19개 전체를 체계적으로 분석한 결과,
어떤 단일 해석적 불변량---$|L(1,\chi)|$, 도체, 차수, 홀짝성,
$\arg\varepsilon(\chi)$---도 $E$-비율을 예측하지 못하는 것으로 드러났다.
특히 함수방정식에 의해 $|L(1,\chi)| = |L(1,\bar\chi)|$ 가 정확히 성립하므로,
가장 자연스러운 후보 역시 배제된다.

켤레 쌍에 걸친 $E$-비율 스펙트럼 $\{1.12, 1.47, 2.05, 4.0, 12.35, 144\}$는
가우스 합 인수의 편각 $\arg W(\chi)$와 국소 영점 간격 미세구조의
복합 효과에 의존하는 것으로 보이며, 이 중 어느 것도 표준 $L$-함수
불변량에 의한 닫힌 형태의 예측을 허용하지 않는다.
이 비동치는 $\xi$-다발 프레임워크의 진정한 특징이다:
곡률 $\kappa$는 $\xi'/\xi$의 \emph{위상 기하학}에 민감하며,
이는 산술적으로 동치인 $\chi$와 $\bar\chi$ 사이에서도 서로 다르다.
\end{remark}
