...
\begin{enumerate}
  ...
  \item 7개의 구조적 대응을 형식화하고, 유비와 증명 사이의 경계를 엄격하게 구분한다 (제~\ref{part:synthesis}부, 제~\ref{sec:correspondence}절).
...
\paragraph{GL(2) 타원곡선 확장 검증.}
예비 GL(2) 결과 (실험~\#52, \#54; 2곡선, rank~0--1)를 conductor $N \in [11, 13766]$의 20곡선 (rank당 5곡선, rank~0--3)으로 확장하였다.
20곡선 중 19곡선이 4가지 $\xi$-다발 진단을 모두 통과:
함수방정식 대칭 ($\log_{10}|\Lambda/\tilde{\Lambda}| = 0.0$),
$\kappa\delta^2$ 안정성 (CV${}<8\%$),
$\sigma$-스윕 국소화 ($\sigma{=}1.0$에서 $\geq 28/30$),
모노드로미 양자화 ($\mathrm{mono}/\pi = 2.000 \pm 0$).
유일한 실패 (11642a1, $N{=}11642$)는 고 conductor 수치 정밀도 한계로 귀속.

PARI/GP \texttt{lfun} 기반 블라인드 예측을 8곡선
(rank당 2곡선, rank~0--3)에 수행하였다
($t \in [2, 50]$, $\Delta t = 0.1$, $\delta = 0.03$,
모노드로미 $r=0.4$, MATCH\_TOL${} = 0.5$):

\begin{center}
\small
\begin{tabular}{llcccccc}
\toprule
곡선 & rank & $N$ & $\varepsilon$ & 영점 & TP & FN & $F_1$ \\
\midrule
11a1 & 0 & 11 & $+1$ & 36 & 36 & 0 & 1.000 \\
43a1 & 0 & 43 & $-1$ & 46 & 46 & 0 & 1.000 \\
37a1 & 1 & 37 & $-1$ & 45 & 45 & 0 & 1.000 \\
79a1 & 1 & 79 & $-1$ & 51 & 51 & 0 & 1.000 \\
389a1 & 2 & 389 & $+1$ & 63 & 63 & 0 & 1.000 \\
571a1 & 2 & 571 & $+1$ & 67 & 67 & 0 & 1.000 \\
5077a1 & 3 & 5077 & $-1$ & 83 & 82 & 1 & 0.994 \\
11197a1 & 3 & 11197 & $-1$ & 89 & 88 & 1 & 0.994 \\
\midrule
\textbf{합계} & & & & \textbf{480} & \textbf{478} & \textbf{2} & \textbf{0.996} \\
\bottomrule
\end{tabular}
\end{center}

8곡선 전원 $F_1 \geq 0.994$, $\mathrm{Precision} = 1.000$ (거짓 양성 0).
성능은 rank 독립: rank~0--1 ($N \leq 79$)은 $F_1 = 1.000$;
rank~2--3 ($N \leq 11197$)은 $F_1 \geq 0.994$.
미탐지 2건 (5077a1 $\gamma \approx 31.67$; 11197a1 $\gamma \approx 39.14$)은
모두 간격${} < 0.16$인 근접 영점 쌍에 위치.
8곡선 총합: $478/480 = 99.6\%$ 검출, 거짓 양성 0.

\paragraph{배경 곡률: $\psi$-보정 공식.}
degree-2 EC $L$-함수의 배경 곡률을
$\kappa_\gamma = |\tfrac{1}{2}\log(N/4\pi^2) + \psi(\sigma+it)|^2$
(digamma 보정)으로 분석하였다.
단순 점근 공식 대비 경험/이론 비율의 CV가
$45.0\% \to 9.3\%$로 개선된다.
비율 $\kappa_{\mathrm{emp}}/\kappa_\gamma$는
$0.79$ (저 conductor, $N=11$)에서 $0.99$ ($N=11197$)까지 분포하며,
체계적 과소는 $|L'/L|^2$ 잔차항에 기인:
$\kappa_{\mathrm{full}} = |g(s)|^2 + O(|L'/L|^2)$.
$N \to \infty$에서 비율$\to 1$은 감마 인자 지배를 확인한다.

\emph{$\xi$-다발 프레임워크가 해석적 rank에 무관하게 GL(2) $L$-함수 영점을
검출하며, 배경 곡률이 보편적 감마 인자 공식으로 지배됨을 시사하는
수치 증거이다.}
