...
\begin{itemize}
  \item \textbf{GL(1)}: Riemann $\zeta$ (mono$/\pi = 2.000$,
    ...
    KS $p = 4.2\times 10^{-14}$)
    and a \emph{complex} Dirichlet character $L(s,\chi_{5,\mathrm{odd}})$
    (mod~5, order~4, odd; $\Lambda(s)$ complex-valued on the critical line:
    mono$/\pi = 2.000$, 20/20 zeros, 30 FP all mono$/\pi = 0.000$;
    KS $p = 4.2\times 10^{-14}$).
    Combined GL(1): 65~TP, 120~FP, zero exceptions.
  ...
  \item \textbf{GL(2)}: elliptic curves 11a1 and 37a1
    ...
    20~TP / 24~FP; Ramanujan~$\Delta$ (weight~12, $\sigma_c=6$):
    20~TP / 30~FP; all with perfect separation.
    The TP/FP separation is identical to GL(1), confirming degree-independent
    and weight-independent monodromy classification.
\end{itemize}
...
\begin{remark}[Relation to Conjecture~\ref{conj:mono_universality}]
\label{rem:conj3}
What was previously labelled ``Conjecture~3'' (now Conjecture~\ref{conj:mono_universality}; the dual criterion
...
GL(1) complex Dirichlet $L(s,\chi_{5,\mathrm{odd}})$ (order~4, $a=1$):
mono$/\pi = 2.000 \pm 0.000$ for 20~TP, $0.000 \pm 0.000$ for 30~FP
(KS $p = 4.2 \times 10^{-14}$; $\Lambda(\tfrac{1}{2}+it)$ complex-valued;
first \emph{complex} character monodromy verification).
Combined across GL(1)--GL(2): \textbf{139~TP, 234~FP}, zero exceptions.
Dedicated monodromy experiments (\#43--\#43j) extend this to
GL(3)--GL(5) symmetric powers and Dedekind zeta:
\textbf{205~TP / 395~FP across 10~$L$-functions
(degree 1--5, weight 1--12, rank 0--1), zero exceptions},
including the Dedekind zeta $\zeta_K$ of $K = \mathbb{Q}(\alpha)$,
$\alpha^3 - \alpha - 1 = 0$ ($\mathrm{Gal} \cong S_3$),
the first verification from a non-abelian Galois extension.
...
\begin{conjecture}[Monodromy Universality]
\label{conj:mono_universality}
Let $L(s,\pi)$ be a primitive $L$-function of degree $d \geq 1$
in the Selberg class, with completed function
$\xi(s,\pi) = \gamma(s,\pi)\,L(s,\pi)$.
Define the $\xi$-bundle connection
$\mathcal{L} = \xi'/\xi$ and the monodromy
\[
  \mathrm{mono}(\gamma_r(\rho))
  \;=\;
  \Bigl|\oint_{\gamma_r(\rho)} \operatorname{Im}\mathcal{L}\,ds\Bigr|
\]
for a circular contour $\gamma_r(\rho)$ of radius~$r$ about~$\rho$.

Then for every nontrivial zero~$\rho$ of $L(s,\pi)$ of multiplicity~$m$:
\[
  \mathrm{mono}(\gamma_r(\rho)) = m\pi
  \qquad
  \text{for all } r < \min_{j\neq\rho}|\rho - \rho_j|,
\]
and for every point~$z$ that is not a zero:
\[
  \mathrm{mono}(\gamma_r(z)) = 0
  \qquad
  \text{for all } r < \min_j |z - \rho_j|.
\]
This holds independently of: degree~$d$, weight~$k$,
conductor~$N$, analytic rank, root number~$\varepsilon$,
and critical line~$\sigma = d/2$.
\end{conjecture}

\begin{remark}[Content and status of
    Conjecture~\textup{\ref{conj:mono_universality}}]
\label{rem:mono_universality_content}
The monodromy identity $\mathrm{mono} = m\pi$ at a zero of
multiplicity~$m$ is a direct consequence of the argument principle
(Theorem~\ref{thm:holonomy}); the non-trivial content is the
\textbf{quantitative exactness} (mono $= m\pi$, not approximately)
and its \textbf{universality} across all tested Selberg class elements.
The conjecture is supported by numerical verification,
\emph{not a proof of universality}:
10~primitive $L$-functions, degree $\{1,2,3,4,5\}$,
weight $\{1,2,3,12\}$, 7~distinct construction paths
(Riemann~$\zeta$, Dirichlet, elliptic curve, Ramanujan~$\Delta$,
sym$^k$, Dedekind~$\zeta_K$ with $\mathrm{Gal}\cong S_3$),
6~distinct critical lines
$\{\tfrac12,1,\tfrac32,2,\tfrac52,6\}$,
\textbf{205~TP / 395~FP, dual criterion pass rate 100\%,
KS $p < 10^{-12}$ in every case, zero exceptions}.
The empirical observation does not constitute a proof that the
identity holds for \emph{all} $L$-functions in the Selberg class;
it is a falsifiable prediction awaiting further verification
(e.g.\ Artin $L$-functions for higher non-abelian groups).
\end{remark}
...
43e & Complex Dirichlet $\chi_{5,\mathrm{odd}}$ monodromy & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 30 FP mono$/\pi{=}0.000$; KS $p{=}4.2{\times}10^{-14}$; dual prec.\ 100\%; $\Lambda$ complex-valued; first complex char. & Dirichlet \\
43f & Ramanujan $\Delta$ monodromy ($\sigma_c{=}6$, weight~12) & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 30 FP mono$/\pi{=}0.000$; KS $p{=}4.2{\times}10^{-14}$; dual prec.\ 100\%; $\sigma_c{=}6$; weight~12; first $w{\geq}3$ dedicated & GL(2) $\Delta$ \\
43g & GL(3) sym$^2$(11a1) monodromy ($\sigma_c{=}1.5$, $d{=}3$) & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 30 FP mono$/\pi{=}0.000$; KS $p{=}7.1{\times}10^{-14}$; dual prec.\ 100\%; first degree-3 dedicated; $\kappa$ TP${\sim}10^4$, FP${\sim}1$--$11$ & GL(3) \\
43h & GL(4) sym$^3$(11a1) monodromy ($\sigma_c{=}2$, $d{=}4$) & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 30 FP mono$/\pi{=}0.000$; KS $p{=}4.2{\times}10^{-14}$; dual prec.\ 100\%; first degree-4 dedicated; $N{=}1331$; $\kappa$ TP${\sim}10^4$, FP${\sim}15$ & GL(4) \\
43i & GL(5) sym$^4$(11a1) monodromy ($\sigma_c{=}2.5$, $d{=}5$) & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 30 FP mono$/\pi{=}0.000$; KS $p{=}4.2{\times}10^{-14}$; dual prec.\ 100\%; first degree-5 dedicated; $N{=}14641{=}11^4$; $\kappa$ TP${\sim}10^4$, FP${\sim}0.3$--$57$ & GL(5) \\
43j & Dedekind $\zeta_K$ monodromy ($K{=}\mathbb{Q}(\alpha)$, $\alpha^3{-}\alpha{-}1{=}0$, $\mathrm{Gal}{\cong}S_3$) & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 101 FP mono$/\pi{=}0.000$; KS $p{=}5.7{\times}10^{-23}$; dual prec.\ 100\%; $|\mathrm{disc}|{=}23$; $\zeta_K{=}\zeta{\cdot}L(s{,}\rho_2)$; 17 zeros from $L(\rho_2)$, 3 from $\zeta$; $\kappa$ ratio $178\times$; first non-abelian Galois origin & Dedekind \\
...
\begin{enumerate}
  ...
  \item \textbf{FP monodromy anatomy (Conjecture~\ref{conj:mono_universality} direct verification).}
  ...
  \item \textbf{Complex Dirichlet character monodromy ($\chi_{5,\mathrm{odd}}$, order~4).}
        The monodromy TP/FP anatomy is extended to a \emph{complex}
        Dirichlet character $\chi\bmod 5$ (order~4, odd, $a=1$), for
        which $\Lambda(\tfrac{1}{2}+it)$ takes genuinely complex values
        on the critical line (no Hardy $Z$-function analogue).
        52~zeros in $T\in[10,100]$ (mpmath, \texttt{dps}$=80$,
        radii $r\in\{0.1, 0.01, 0.001\}$, $n_{\mathrm{steps}}=256$).
        20~TP yield mono$/\pi = 2.000 \pm 0.000$;
        30~FP yield mono$/\pi = 0.000 \pm 0.000$
        (KS $p = 4.2 \times 10^{-14}$; dual precision 100\%).
        Curvature: $\kappa_{\mathrm{TP}} \sim 10^{28}$--$10^{33}$,
        $\kappa_{\mathrm{FP}} \sim 0.005$--$2.1$ (26--31 orders of magnitude gap).
        This is the first monodromy verification for a non-self-dual
        $L$-function, confirming that the $\arg(\Lambda)$-accumulation
        monodromy is well-defined and yields perfect TP/FP separation
        even when $\Lambda$ is complex-valued.
        Combined dedicated results (\#43, \#43b, \#43c, \#43d, \#43e):
        \textbf{105~TP / 174~FP across 5~$L$-functions
        (degree 1--2, rank 0--1, real and complex characters), zero exceptions.}
        \textcolor{ForestGreen}{[confirmed 2026-04-29; $\chi_{5,\mathrm{odd}}$ complex char.; numerical verification, not proof]}
  \item \textbf{Ramanujan $\Delta$ dedicated monodromy at $\sigma_c = 6$ (weight~12).}
        The monodromy TP/FP anatomy is extended to the Ramanujan~$\Delta$
        $L$-function ($L(s,\Delta)$, weight~12, level~1, root number
        $\varepsilon = +1$), whose critical line lies at $\sigma_c = k/2 = 6$
        --- the highest critical-line value tested and far from the
        classical $\sigma = \tfrac{1}{2}$.
        20~zeros in $T \leq 50$ (PARI \texttt{lfun} with
        \texttt{ramanujantau} coefficients, \texttt{realprecision}$=100$,
        $n_{\mathrm{steps}} = 128$, radii $r \in \{0.1, 0.01, 0.001\}$).
        20~TP yield mono$/\pi = 2.000 \pm 0.000$;
        30~FP yield mono$/\pi = 0.000 \pm 0.000$
        (KS $p = 4.2 \times 10^{-14}$; dual precision 100\%).
        Curvature: $\kappa_{\mathrm{TP}} \sim 10^{4}$,
        $\kappa_{\mathrm{FP}} \sim 0.03$--$3.3$ (3--5 orders of magnitude gap).
        This is the first dedicated monodromy TP/FP verification for a
        \emph{high-weight} modular form ($w = 12$, vs.\ $w \leq 2$
        in all prior experiments), confirming that the perfect separation
        is weight-independent and critical-line-independent.
        Weight coverage: $\{1, 2, 12\}$; critical-line coverage:
        $\{\tfrac{1}{2}, 1, 6\}$.
        Combined dedicated results (\#43, \#43b--\#43f):
        \textbf{125~TP / 204~FP across 6~$L$-functions
        (degree 1--2, weight 1--12, rank 0--1, real and complex characters),
        zero exceptions.}
        \textcolor{ForestGreen}{[confirmed 2026-04-29; $\Delta$ weight~12, $\sigma_c{=}6$; numerical verification, not proof]}
  \item \textbf{GL(3) sym$^2$(11a1) dedicated monodromy at $\sigma_c = 1.5$ (degree~3).}
        The monodromy TP/FP anatomy is extended to the first
        \emph{degree-3} $L$-function: $L(s, \mathrm{sym}^2(11\mathrm{a}1))$
        (conductor~121, motivic weight~3, $\Gamma$-vector $[0,0,1]$,
        root number $\varepsilon = +1$, critical line $\sigma_c = 1.5$).
        64~zeros in $T \leq 50$ (PARI \texttt{lfunsympow},
        \texttt{realprecision}$=100$, $n_{\mathrm{steps}} = 128$,
        radii $r \in \{0.1, 0.01, 0.001\}$).
        20~TP yield mono$/\pi = 2.000 \pm 0.000$;
        12~FP yield mono$/\pi = 0.000 \pm 0.000$
        (KS $p = 8.9 \times 10^{-9}$; dual precision 100\%).
        Curvature: $\kappa_{\mathrm{TP}} \sim 10^{4}$,
        $\kappa_{\mathrm{FP}} \sim 1.7$--$6.8$ (3 orders of magnitude gap).
        The FP count (12) is below the usual target ($\geq 20$) because
        GL(3) zeros are denser ($64$ in $T \leq 50$), leaving fewer
        non-zero gaps for FP placement; all 12~FP that were tested
        show perfect mono$/\pi = 0$.
        Degree coverage: $\{1, 2, 3\}$; this confirms that the
        monodromy TP/FP separation extends beyond GL(2) to GL(3),
        with a qualitatively different $\Gamma$-factor structure.
        Combined dedicated results (\#43, \#43b--\#43g):
        \textbf{145~TP / 234~FP across 7~$L$-functions
        (degree 1--3, weight 1--12, rank 0--1), zero exceptions.}
        \textcolor{ForestGreen}{[confirmed 2026-04-29; sym$^2$(11a1) degree~3, $\sigma_c{=}1.5$; numerical verification, not proof]}
  \item \textbf{GL(4) sym$^3$(11a1) dedicated monodromy at $\sigma_c = 2$ (degree~4).}
        The monodromy TP/FP anatomy is extended to the first
        \emph{degree-4} $L$-function: $L(s, \mathrm{sym}^3(11\mathrm{a}1))$
        (conductor~$1331 = 11^3$, $\Gamma$-vector $[-1,0,0,1]$,
        critical line $\sigma_c = 2$, FE: $\Lambda(s) = \Lambda(4-s)$).
        92~zeros in $T \leq 50$, 144~in $T \leq 70$
        (PARI \texttt{lfunsympow}, \texttt{realprecision}$=100$,
        $n_{\mathrm{steps}} = 128$, radii $r \in \{0.1, 0.01, 0.001\}$;
        \texttt{lfuncheckfeq}$=-62$ digits).
        20~TP yield mono$/\pi = 2.000 \pm 0.000$ at all three radii;
        30~FP yield mono$/\pi = 0.000 \pm 0.000$
        (KS $p = 4.2 \times 10^{-14}$; dual precision 100\%).
        Curvature: $\kappa_{\mathrm{TP}} \sim 10^{4}$,
        $\kappa_{\mathrm{FP}} \sim 0.3$--$78$ (2--4 orders of magnitude gap).
        Note: 11a1 serves as anchor for $\mathrm{sym}^k$ tower:
        $k=1$ (C-408, $d{=}2$), $k=2$ (C-412, $d{=}3$),
        $k=3$ (C-413, $d{=}4$)---all yield perfect separation.
        Degree coverage: $\{1, 2, 3, 4\}$; critical-line coverage:
        $\{\tfrac{1}{2}, 1, \tfrac{3}{2}, 2, 6\}$ (5~distinct values).
        Combined dedicated results (\#43, \#43b--\#43h):
        \textbf{165~TP / 264~FP across 8~$L$-functions
        (degree 1--4, weight 1--12, rank 0--1), zero exceptions.}
        \textcolor{ForestGreen}{[confirmed 2026-04-30; sym$^3$(11a1) degree~4, $\sigma_c{=}2$; numerical verification, not proof]}
  \item \textbf{GL(5) sym$^4$(11a1) dedicated monodromy at $\sigma_c = 2.5$ (degree~5).}
        The monodromy TP/FP anatomy is extended to the first
        \emph{degree-5} $L$-function: $L(s, \mathrm{sym}^4(11\mathrm{a}1))$
        (conductor~$14641 = 11^4$, $\Gamma$-vector $[-2,-1,0,0,1]$,
        critical line $\sigma_c = 2.5$, FE: $\Lambda(s) = \Lambda(5-s)$).
        121~zeros in $T \leq 50$, 187~in $T \leq 70$
        (PARI \texttt{lfunsympow}, \texttt{realprecision}$=150$,
        $n_{\mathrm{steps}} = 128$, radii $r \in \{0.1, 0.01, 0.001\}$;
        \texttt{lfuncheckfeq}$=-43$ digits).
        20~TP yield mono$/\pi = 2.000 \pm 0.000$ at all three radii;
        30~FP yield mono$/\pi = 0.000 \pm 0.000$
        (KS $p = 4.2 \times 10^{-14}$; dual precision 100\%).
        Curvature: $\kappa_{\mathrm{TP}} \sim 10^{4}$,
        $\kappa_{\mathrm{FP}} \sim 0.3$--$57$ (2--4 orders of magnitude gap).
        The $\mathrm{sym}^k$(11a1) tower now spans $k=1,\ldots,4$
        (degree $d=2,\ldots,5$), all yielding perfect separation.
        Degree coverage: $\{1, 2, 3, 4, 5\}$; critical-line coverage:
        $\{\tfrac{1}{2}, 1, \tfrac{3}{2}, 2, \tfrac{5}{2}, 6\}$ (6~distinct values).
        Combined dedicated results (\#43, \#43b--\#43j):
        \textbf{205~TP / 395~FP across 10~$L$-functions
        (degree 1--5, weight 1--12, rank 0--1), zero exceptions.}
        The latest addition (\#43j) is the Dedekind zeta $\zeta_K$ of
        $K = \mathbb{Q}(\alpha)$ ($\alpha^3 - \alpha - 1 = 0$,
        $\mathrm{disc} = -23$, $\mathrm{Gal}(\widetilde{K}/\mathbb{Q}) \cong S_3$):
        20~TP ($17$ from $L(s,\rho_2)$, $3$ from $\zeta$),
        101~FP, mono$/\pi = 2.000 \pm 0$ vs.\ $0.000 \pm 0$,
        KS $p = 5.7 \times 10^{-23}$, $\kappa$ ratio $178\times$.
        This is the first verification from a \emph{Dedekind zeta}
        (non-abelian Galois extension), confirming construction-path independence.
        \textcolor{ForestGreen}{[confirmed 2026-04-30; sym$^4$(11a1) degree~5, $\sigma_c{=}2.5$; numerical verification, not proof]}
...
Conjecture~\ref{conj:mono_universality} asserts that $\kappa$ is large near zeros and small away from
...
false positives (cf.\ Conjecture~\ref{conj:mono_universality}).
...
Conjecture~\ref{conj:mono_universality} extends to GL(3).  Monodromy quantisation yields mono/$\pi =
