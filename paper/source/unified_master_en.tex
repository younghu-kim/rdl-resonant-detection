...
($p = 9.7 \times 10^{-17}$).
Combined across GL(1)--GL(2): 79~TP, 150~FP, zero exceptions.
...
\begin{enumerate}
  ...
  \item \textbf{FP monodromy anatomy (Conjecture~3 direct verification).}
        ...
        A direct contour-integral monodromy measurement on 25~true zeros
        (TP) and 60~false-positive candidates (FP, midpoints + random
        non-zeros) in $t\in[14,300]$ confirms the binary classifier:
        TP $|\mathrm{mono}|/\pi = 2.000 \pm 0.000$ (winding number~1,
        phase change $= 2\pi$), FP $|\mathrm{mono}|/\pi = 0.000 \pm 0.000$
        (KS $p = 4.2 \times 10^{-14}$).
        The dual criterion ($\kappa + \mathrm{mono}$) achieves
        $100\%$ precision across all 85~candidates
        (25~TP, 60~FP) with zero exceptions.
        The range $T\in[14,300]$ spans low- to high-density
        regimes, confirming robustness to varying zero density.
        \textcolor{ForestGreen}{[confirmed 2026-04-28; $\zeta$ $t\in[14,300]$; 25 TP / 60 FP; numerical verification, not proof]}
