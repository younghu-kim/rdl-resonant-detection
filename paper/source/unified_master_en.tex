...
\begin{enumerate}
  ...
  \item \textbf{Higher-conductor Dirichlet extension (exhaustive per-character verification).}
        ...
        monodromy $|\mathrm{mono}|/\pi = 1.000000$ (all ${\sim}2\,550$~zeros),
        ...
        $q = 3, 4, 5, 7, 8, 11$, totalling ${\sim}2\,590$~zeros,
        all satisfying the four-property bundle framework.
...
($\chi \bmod 3,4,5,7,8,11$; 21~primitive characters, ${\sim}2590$~zeros), (v)~arithmetic subleading structure and
...
\midrule
\multicolumn{2}{l}{\textbf{Cumulative (all $q$)}} & \textbf{$\sim$2590} & & & & \\
\bottomrule
\end{tabular}
\end{table}
...
conditionally PASS due to scan artefact).
...
Four conjugate pairs at $q{=}11$ reveal a continuous spectrum of
$E$-ratio asymmetry:
...
for $q=7,8,11$ all ${\sim}2\,550$ zeros satisfy the same condition
