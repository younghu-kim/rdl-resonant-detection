...
Blind zero prediction was further tested on three elliptic curves
using PARI/GP \texttt{lfun} ($t \in [2, 50]$, $\Delta t = 0.1$,
$\delta = 0.03$):
11a1 (rank~0, $F_1 = 1.000$, 36/36~TP),
389a1 (rank~2, $F_1 = 1.000$, 63/63~TP),
and 5077a1 (rank~3, $F_1 = 0.994$, 82/83~TP).
All three achieve $\mathrm{Precision} = 1.000$ (zero false positives).
The single missed zero (5077a1, $\gamma \approx 31.67$) lies in
a close pair where $\Delta t = 0.1$ merges adjacent $\kappa$-peaks.
Combined with the earlier mpmath experiments, the five-curve GL(2)
aggregate achieves $181/182 = 99.5\%$ detection with FP${} = 0$.
...
consistent with the universality thesis established in
Part~\ref{part:evidence}.}
\end{remark}
