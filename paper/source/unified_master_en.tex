...
\begin{itemize}
  ...
  \item \textbf{GL(2)}: elliptic curves 11a1 and 37a1
    ...
    20~TP / 24~FP; Ramanujan~$\Delta$ (weight~12, $\varepsilon=+1$, $\sigma=6$):
...
GL(2) Ramanujan $\Delta$ ($N=1$, weight~12, $\varepsilon=+1$):
mono$/\pi = 2.000 \pm 0.000$ for 20~TP, $0.000 \pm 0.000$ for 30~FP
(KS $p = 4.2 \times 10^{-14}$; $\sigma = 6$ in PARI arithmetic normalisation,
i.e.\ $\Lambda(s) = \Lambda(12-s)$; first weight-12 dedicated monodromy verification).
Combined across GL(1)--GL(2): \textbf{159~TP, 264~FP}, zero exceptions.
...
43f & Ramanujan $\Delta$ monodromy (weight~12, $\sigma{=}6$) & \textbf{E} & 20 TP mono$/\pi{=}2.000{\pm}0$; 30 FP mono$/\pi{=}0.000$; KS $p{=}4.2{\times}10^{-14}$; dual prec.\ 100\%; $\sigma_c{=}6$ (weight 12); first weight-12 dedicated & GL(2) $\Delta$ \\
...
\begin{enumerate}
  ...
  \item \textbf{Ramanujan $\Delta$ monodromy (weight~12, level~1, $\varepsilon=+1$).}
        The monodromy TP/FP anatomy is extended to the Ramanujan~$\Delta$
        cuspform of weight~12, the unique normalised newform at level~1.
        This is the first monodromy verification at a critical line
        far from $\sigma = 1/2$ or $\sigma = 1$: in PARI's arithmetic
        normalisation, $\Lambda(s) = \Lambda(12-s)$ with critical line
        $\sigma = 6$.
        20~zeros in $T \leq 50$ (PARI \texttt{lfunzeros},
        \texttt{realprecision}$=100$, 500 \texttt{ramanujantau} coefficients;
        radii $r\in\{0.1, 0.01, 0.001\}$, $n_{\mathrm{steps}}=128$).
        20~TP yield mono$/\pi = 2.000 \pm 0.000$;
        30~FP yield mono$/\pi = 0.000 \pm 0.000$
        (KS $p = 4.2 \times 10^{-14}$; dual precision 100\%).
        Curvature: $\kappa_{\mathrm{TP}} \sim 10^{4}$,
        $\kappa_{\mathrm{FP}} \sim 0.06$--$3.3$ (3--4 orders of magnitude gap).
        Weight coverage now spans $\{1, 2, 12\}$---a factor-of-six jump
        from the previous maximum---establishing that monodromy separation
        is independent of the weight of the underlying modular form.
        Combined dedicated results (\#43, \#43b, \#43c, \#43d, \#43e, \#43f):
        \textbf{125~TP / 204~FP across 6~$L$-functions
        (degree 1--2, weight 1--12, rank 0--1, real and complex characters), zero exceptions.}
        \textcolor{ForestGreen}{[confirmed 2026-04-29; Ramanujan $\Delta$ weight 12, $\sigma{=}6$; numerical verification, not proof]}
