...
\begin{enumerate}
  ...
  \item We formalise seven structural correspondences and rigorously
        ...
        (Part~\ref{part:evidence}, Section~\ref{sec:correspondence}).
...
\begin{remark}[Extended EC verification: 20 curves, ranks~0--3]
\label{rem:ec_extended_verification}
The preliminary GL(2) results above (experiments~\#52, \#54; 2~curves,
ranks~0--1) were extended to 20~elliptic curves spanning ranks~0--3
with conductors $N \in [11, 13766]$ (5~curves per rank).
Of 20~curves, 19~pass all four $\xi$-bundle diagnostics:
functional-equation symmetry ($\log_{10}|\Lambda/\tilde{\Lambda}| = 0.0$),
$\kappa\delta^2$ stability (CV${}<8\%$),
$\sigma$-sweep localization ($\geq 28/30$ detections at $\sigma=1.0$),
and monodromy quantization ($\mathrm{mono}/\pi = 2.000 \pm 0$).
The single failure (11642a1, $N{=}11642$, rank~3)
has $\kappa\delta^2$~CV${}=32\%$, attributed to numerical precision
limits at high conductor.

Blind zero prediction was tested on eight elliptic curves
(two per rank, ranks~0--3) using PARI/GP \texttt{lfun}
($t \in [2, 50]$, $\Delta t = 0.1$, $\delta = 0.03$,
monodromy $r=0.4$, MATCH\_TOL${} = 0.5$):

\begin{center}
\small
\begin{tabular}{llcccccc}
\toprule
Curve & rank & $N$ & $\varepsilon$ & Zeros & TP & FN & $F_1$ \\
\midrule
...
\midrule
\textbf{Total} & & & & \textbf{480} & \textbf{478} & \textbf{2} & \textbf{0.996} \\
\bottomrule
\end{tabular}
\end{center}

All eight curves achieve $F_1 \geq 0.994$ with
$\mathrm{Precision} = 1.000$ (zero false positives).
Performance is rank-independent: ranks~0--1 ($N \leq 79$)
achieve $F_1 = 1.000$; ranks~2--3 ($N \leq 11197$)
achieve $F_1 \geq 0.994$.  The two missed zeros
(5077a1, $\gamma \approx 31.67$; 11197a1, $\gamma \approx 39.14$)
both lie in close pairs with gap${} < 0.16$, where
$\Delta t = 0.1$ merges adjacent $\kappa$-peaks.
The eight-curve aggregate achieves
$478/480 = 99.6\%$ detection with FP${} = 0$.

\paragraph{Background curvature: $\psi$-corrected formula.}
The background curvature for degree-2 EC $L$-functions was analysed
using the full digamma-corrected formula
$\kappa_\gamma = |\tfrac{1}{2}\log(N/4\pi^2) + \psi(\sigma+it)|^2$,
where $\psi$ denotes the digamma function.
Compared to the simpler asymptotic approximation
$\kappa_{\mathrm{simple}} = (\tfrac{1}{2}\log(N/4\pi^2)+\gamma)^2
+ \pi^2/4$,
the $\psi$-correction reduces the empirical-to-theoretical ratio
CV from $45.0\%$ to $9.3\%$ across the eight curves.
The ratio $\kappa_{\mathrm{emp}}/\kappa_\gamma$ ranges from
$0.79$ (low conductor, $N=11$) to $0.99$ ($N=11197$),
with the systematic undershoot attributable to the
$|L'/L|^2$ residual:
$\kappa_{\mathrm{full}} = |g(s)|^2 + O(|L'/L|^2)$,
where $g(s) = \tfrac{1}{2}\log(N/4\pi^2) + \psi(s)$
is the gamma-factor contribution.
The scaling $\kappa_{\mathrm{emp}}/\kappa_\gamma \to 1$
as $N \to \infty$ confirms gamma-factor dominance for large
conductor, consistent with the GL(1) curvature analysis.

\emph{Numerical evidence suggests that the $\xi$-bundle framework
detects zeros of GL(2) $L$-functions regardless of analytic rank,
with background curvature governed by a universal
gamma-factor formula.  This is consistent with the universality
thesis established in Part~\ref{part:evidence}.}
\end{remark}
...
\midrule
37 & Dirichlet $\chi\bmod 7$ (high conductor, 5 chars.) & \textbf{E} & 689 zeros ($t{\in}[10,200]$, C-345); mono 689/689; 5/5 ALL-PASS; cond.\ effect B/E & Dirichlet \\
...
\begin{enumerate}
  ...
  \item \emph{Prior conductor table ($q=3,4,5,7$).}
        ...
        per-conductor zero totals: 12, 13, 13, 689 zeros ($q{=}7$: $t{\in}[10,200]$, 5 chars., C-345) for $q=3,4,5,7$ respectively).
\end{enumerate}
