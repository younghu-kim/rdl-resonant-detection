#!/usr/bin/env python3
"""
[Project RDL] #72v3 — GL(4) sym³(Δ) FE 해결 + 재측정
=============================================================================
#72v2 실패 원인 + 수정:
  1. N_COEFF=800 부족 → 3000 (degree 비례 3.75×)
  2. float64 정밀도 부족 → AFE 핵심 mpmath dps=60
  3. 비자명 FE Re(s)=2 → Re(s)=3~5 (감마 스케일 안정)
  4. κ_near 전 영점 → t<20 영점만 (고t 불안정 배제)
  5. LMFDB 교차검증 (첫 영점 t₁≈5.825)
...
OUTFILE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "..", "results", "gl4_sym3_delta_72v3.txt")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
log("=" * 72)
log("결과 #72v3 — GL(4) sym³(Δ) FE 해결 + 재측정")
log("=" * 72)
log(f"시각: {time.strftime('%Y-%m-%d %H:%M:%S')}")
log(f"degree=4, conductor N=1, root number ε={EPSILON}")
log(f"L-함수: L(s, sym³Δ), μ=(0,1,11,12)")
log(f"★ 수정: N_COEFF={N_COEFF}, mpmath dps={mpmath.mp.dps}, FE at Re(s)=3~5")
log(f"c_shift={C_SHIFT}, N_GH={N_GH}")
...
# ── Step 1: ★ 비자명 FE 검증 (mpmath, Re(s)=3~5) ──
log(f"\n{'='*72}")
log(f"[Step 1] ★★★ 비자명 FE 검증 (핵심! — #72v2 실패 원인 해결)")
log(f"{'='*72}")
log(f"  방법: L_direct(s) vs L_AFE(s), 전부 mpmath dps={mpmath.mp.dps}")
log(f"  검증점: Re(s)=3,4,5 (Re(s)=2 대신 — 수렴 빠름)")
...
# 검증점: Re(s) = 3, 4, 5 (각각 2~3개 Im(s) 값)
fe_test_pts = [
    mpmath.mpc(3, 5),  mpmath.mpc(3, 10), mpmath.mpc(3, 15),
    mpmath.mpc(4, 5),  mpmath.mpc(4, 10),
    mpmath.mpc(5, 5),  mpmath.mpc(5, 10),
]
...
for s in fe_test_pts:
    ...
    try:
        ...
        log(f"  s={float(s.real):.0f}+{float(s.imag):.0f}i: "
            f"|L_dir|={float(abs(L_dir)):.6e}, |L_afe|={float(abs(L_afe)):.6e}, "
            f"rel={rel:.2e} {ok}  ({time.time()-ts:.1f}s)")
    except Exception as e:
        log(f"  s={float(s.real):.0f}+{float(s.imag):.0f}i: ERROR {e}")
...
# ── 종합 ──
log(f"\n{'='*72}")
log(f"종합 판정 — GL(4) sym³(Δ) #72v3")
log(f"{'='*72}")

log(f"\n  [P0] ★ 비자명 FE 검증 (Re(s)=3~5, mpmath dps={mpmath.mp.dps})")
