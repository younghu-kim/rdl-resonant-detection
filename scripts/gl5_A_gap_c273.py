#!/usr/bin/env python3
"""
[사이클 #273] GL(5) Sym⁴(11a1) A-gap 상관 + Prop 12 검증
  GL(4) C-269 스크립트 확장. degree 5까지 보편성 검증.

  Sym⁴(11a1): gammaV=[-2,-1,0,0,1], k=5, N=14641, ε=+1
  center = k/2 = 2.5 (PARI 정규화)
...
try:
    import cypari2
    pari = cypari2.Pari()
    pari.allocatemem(4000000000)  # 4GB (GL(5) 필요)
    pari.set_real_precision(100)
    print("cypari2 OK (4 GB, precision=100)")
...
T_MAX = 55.0
T_MIN = 2.0
CENTER = 2.5  # k=5 → center=k/2=2.5

RESULT_PATH = os.path.expanduser('~/Desktop/gdl_unified/results/gl5_A_gap_c273.txt')
...
def get_zeros_sym4(coeffs, t_max):
    """PARI lfunzeros로 GL(5) Sym⁴ 영점 수집."""
    pari(f'E_sym4 = ellinit({coeffs})')
    pari('L_sym4 = lfunsympow(E_sym4, 4)')
    pari(f'Li_sym4 = lfuninit(L_sym4, [0, {t_max + 2}])')
    pari(f'zv_sym4 = lfunzeros(Li_sym4, {t_max})')
    n = int(str(pari('#zv_sym4')))
    zeros = []
    for i in range(1, n + 1):
        t = pf(pari(f'zv_sym4[{i}]'))
...
def analyze_sym4(curve):
    ...
    print("[영점]...")
    t0 = time.time()
    all_zeros = get_zeros_sym4(curve['coeffs'], T_MAX)
...
# ===== 메인 =====
print("=" * 70)
print("[사이클 #273] GL(5) Sym⁴ A-gap + Prop 12")
...
CURVES = [
    {'name': 'sym4_11a1',
     ...
     'mu': [-2, -1, 0, 0, 1],
     'degree': 5,
     'label': 'Sym⁴(11a1) (N=14641, d=5)'},
]

all_results = {}
for curve in CURVES:
    try:
        result = analyze_sym4(curve)
...
# 비교표 (GL(1)~GL(5) 전체)
print("=" * 70)
print("  전체 비교표: A-gap 보편성 (GL(1)~GL(5))")
print("=" * 70)

# 기존 결과 (A_Λ → gap_right, A_L → gap_min)
REF_A_LAMBDA = [
...
if all_results and all_neg:
    vt = f"★★★★ GL(1)~GL(5) degree-독립 보편성 ({n_total}/{n_total} 음)"
else:
    vt = "미완 또는 반례 발견"
print(f"\n  최종: {vt}")
...
deg_h1a = {
    1: 0.867,   # ζ(s) C-265
    2: 0.82,    # 11a1/37a1 평균 추정
    3: 0.748,   # Sym²(11a1)/Sym²(37a1) C-264
    4: 0.67,    # Sym³(11a1)/Sym³(37a1) C-269
}
...
for nm, r in all_results.items():
    ...
    print(f"    → degree 경향: {'감소 지속' if r['prop12_H1_frac'] < 0.67 else '포화 또는 반전'}")

# 저장
with open(RESULT_PATH, 'w') as f:
    f.write("=" * 70 + "\n")
    f.write("[사이클 #273] GL(5) Sym⁴ A-gap + Prop 12\n")
    f.write(f"  영점 합 방식 (A_L), T=[{T_MIN},{T_MAX}], center={CENTER}\n")
    f.write(f"  대상: Sym⁴(11a1) d=5 N=14641\n")
    f.write("=" * 70 + "\n\n")
