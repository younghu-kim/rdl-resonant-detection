...
MONO_NSTEPS = 256  # 위상 추적 안전 마진 (기본 64 → 256)

mpmath.mp.dps = 80  # 80자리 정밀도 (bundle_utils._near_zero 임계값에 충분)
...
def train_and_get_predictions(dataset, seed):
    ...
    # 진짜 영점 (compute_zeros_in_range가 dps를 20으로 내리므로 복원 필수)
    true_zeros = compute_zeros_in_range(T_MIN, T_MAX)
    mpmath.mp.dps = 80  # dps 복원 — monodromy_contour._near_zero에 필수
...
def main():
    ...
    for i, t_det in enumerate(tp_unique[:20]):  # 최대 20개
        ...
        monos_by_r = {}
        for r in MONO_RADII:
            monos_by_r[r] = monodromy_contour(t_zero, radius=r, n_steps=MONO_NSTEPS)
        mono = monos_by_r[0.1]  # 가장 안정적인 반지름 사용 (n_steps=256)
    ...
    for i, t in enumerate(fp_unique[:30]):  # 최대 30개
        ...
        mono = monos_by_r[0.1]  # 가장 안정적인 반지름 사용
