...
"""
=============================================================================
[RDL 실험 #127] Weight 20 Cusp Form S₂₀(SL₂(Z)) — ξ-다발 4성질 검증
=============================================================================
배경:
  #125 (w=12) slope=2.0008, #126 (w=16) slope=1.9989 → weight 불변성 2데이터.
  세 번째 데이터 포인트로 weight 불변성 통계적 강화.

  S₂₀(SL₂(Z))의 유일 정규화 Hecke 고유형식: f₂₀ = Δ·E₈
  E₈(z) = 1 + 480·Σ σ₇(n)qⁿ (weight 8 Eisenstein)
  a₂₀(n) = Σ_{j=1}^{n} τ(j)·e₈(n-j), e₈(0)=1, e₈(m)=480·σ₇(m)

  L(s, f₂₀) = Σ a₂₀(n)/nˢ, 임계선 Re(s) = 10
  Λ(s) = (2π)⁻ˢ Γ(s) L(s, f₂₀), FE: Λ(s) = Λ(20-s), ε = +1

성공 기준:
  κδ² slope = 2.0 ± 0.01 + R² > 0.9999 → weight 불변성 ★★★ 강화 (3데이터)
  slope > 2.1 또는 < 1.9 → weight 의존성 발견 → B-32 신설

결과: results/weight20_cusp_127.txt
=============================================================================
"""
...
RESULT_FILE = os.path.join(RESULT_DIR, 'weight20_cusp_127.txt')
...
SIGMA_CRIT = 10       # 임계선: Re(s) = k/2 = 10
K_WEIGHT = 20
N_LEVEL = 1
N_TERMS = 20          # 계수 성장 O(n^{9.5})이므로 여유 확보
DELTAS = [0.005, 0.008, 0.01, 0.015, 0.02, 0.03, 0.05, 0.10]

log("=" * 72)
log("[실험 #127] Weight 20 Cusp Form S₂₀(SL₂(Z)) — ξ-다발 4성질 검증")
log("=" * 72)
...
# Step 1b: σ₇(n) 계산
def sigma7(n):
    """σ₇(n) = sum of 7th powers of divisors of n"""
    s = 0
...
# Step 1c: E₈ 계수: e₈(0) = 1, e₈(m) = 480·σ₇(m)
E8_COEFF = {0: 1}
for m in range(1, N_TERMS + 1):
    E8_COEFF[m] = 480 * sigma7(m)
...
for n in range(1, N_TERMS + 1):
    ...
    for j in range(1, n + 1):
        val += TAU_INT[j] * E8_COEFF[n - j]
    A20_INT[n] = val

# a₂₀(n) 교차검증 (수동 계산값)
# a₂₀(1) = τ(1)·1 = 1
# a₂₀(2) = τ(2) + τ(1)·480·σ₇(1) = -24 + 480 = 456
# a₂₀(3) = τ(3) + τ(2)·480·1 + τ(1)·480·σ₇(2) = 252 + (-24)(480) + 1·480·129
#         = 252 - 11520 + 61920 = 50652
KNOWN_A20 = {1: 1, 2: 456, 3: 50652}
a20_ok = True
for n, expected in sorted(KNOWN_A20.items()):
    computed = A20_INT[n]
    match = "✓" if computed == expected else "✗"
    if computed != expected:
        a20_ok = False
    log(f"  a₂₀({n:2d}) = {computed:>15d}  (기대: {expected:>15d}) {match}")

# σ₇(2) = 1+128 = 129 확인
log(f"  σ₇(2) = {sigma7(2)} (기대: 129)")

# Ramanujan-Petersson 검증: |a₂₀(p)| ≤ 2·p^{19/2} (Deligne)
log(f"  Deligne 한계 검증:")
for p in [2, 3, 5, 7, 11, 13]:
    if p > N_TERMS:
        break
    bound = 2 * p ** 9.5
    actual = abs(A20_INT[p])
    ok = actual <= bound * 1.01
    log(f"    p={p:2d}: |a₂₀(p)|={actual:>15d}, 한계={bound:>18.0f} {'✓' if ok else '✗'}")

# 나머지 계수 출력
log(f"  전체 a₂₀(n) (n=1..{min(15, N_TERMS)}):")
for n in range(1, min(16, N_TERMS + 1)):
    log(f"    a₂₀({n:2d}) = {A20_INT[n]}")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 2. Λ(s) = Σ a₂₀(n) [Γ(s,2πn)·(2πn)⁻ˢ + Γ(20-s,2πn)·(2πn)⁻⁽²⁰⁻ˢ⁾]
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

log(f"{T()} [2/6] Λ(s) 구현 (Mellin 분할, 지수적 수렴)...")

def completed_w20(s, N=N_TERMS):
    """완비 L-함수 Λ(s, f₂₀) via Mellin integral split.
    Λ(s) = Σ_{n=1}^N a₂₀(n) [Γ(s, 2πn)·(2πn)⁻ˢ + Γ(20-s, 2πn)·(2πn)⁻⁽²⁰⁻ˢ⁾]
    """
    result = mpmath.mpc(0)
    two_pi = 2 * mpmath.pi
    s20 = K_WEIGHT - s
    for n in range(1, N + 1):
        ...
        g2 = mpmath.gammainc(s20, x)
        xps = mpmath.power(x, s)
        xps20 = mpmath.power(x, s20)
        result += a_n * (g1 / xps + g2 / xps20)
    return result


def connection_w20(s, N=N_TERMS):
    """접속 L(s) = Λ'/Λ (수치 미분)"""
    h = mpmath.mpf(10) ** (-20)
    val = completed_w20(s, N)
    if abs(val) < mpmath.mpf(10) ** (-mpmath.mp.dps + 10):
        return mpmath.mpc(1e10, 0)
    dval = (completed_w20(s + h, N) - completed_w20(s - h, N)) / (2 * h)
    return dval / val
...
test_points = [
    mpmath.mpc(5, 10), mpmath.mpc(7, 15), mpmath.mpc(9, 20),
    mpmath.mpc(11, 12), mpmath.mpc(12, 25),
]
fe_max_err = 0
for sp in test_points:
    v1 = completed_w20(sp)
    v2 = completed_w20(K_WEIGHT - sp)
...
def scan_segment(t_min, t_max, n_pts, dps_val):
    ...
    for t in ts:
        ...
        v = float(mpmath.re(completed_w20(s)))
        vals.append(v)
    intervals = []
...
# 구간별 스캔 — weight 증가로 Γ 감쇠 더 빠름, dps 추가 확보
segments = [
    (1.0, 20.0, 600, 35),
    (20.0, 35.0, 500, 55),
    (35.0, 50.0, 500, 70),
...
for a, b in all_intervals:
    va = float(mpmath.re(completed_w20(sigma + 1j * mpmath.mpf(str(a)))))
    for _ in range(60):
        mid = (a + b) / 2
        vm = float(mpmath.re(completed_w20(sigma + 1j * mpmath.mpf(str(mid)))))
        if va * vm < 0:
            b = mid
        else:
            a = mid
...
n_zeros = len(zeros)
log(f"  발견된 영점: {n_zeros}개 (dps=70 이분법)")
...
for t0 in zeros[:5]:
    ...
    v1 = completed_w20(s_test)
    v2 = completed_w20(K_WEIGHT - s_test)
    denom = max(abs(v1), abs(v2), mpmath.mpf(1e-100))
    err = float(abs(v1 - v2) / denom)
    fe_errs.append(err)
...
mpmath.mp.dps = 70  # weight 증가 → dps 추가 확보
...
for iz in range(n_kappa_zeros):
    ...
    for delta in DELTAS:
        ...
        L_plus = connection_w20(s_plus)
        L_minus = connection_w20(s_minus)
...
for iz in range(n_mono):
    t0 = zeros[iz]
    mono = monodromy_w20(t0, radius=0.3, n_steps=96)
...
def count_sign_changes(sigma_val, t_min=5.0, t_max=30.0, n_points=500):
    ...
    for t in ts:
        ...
        val = float(mpmath.re(completed_w20(s)))
...
# σ 값은 임계선 Re(s)=10 주위로 설정
sigmas_test = [8.0, 9.0, 9.5, 10.0, 10.5, 11.0, 12.0]
sigma_jumps = {}
for sig in sigmas_test:
    nj = count_sign_changes(sig)
    sigma_jumps[sig] = nj
    marker = " ← critical" if sig == 10.0 else ""
    log(f"  σ={sig:.1f}: 부호변화 {nj:3d}{marker}")

crit_jumps = sigma_jumps.get(10.0, 0)
...
def L_euler_product_w20(s, primes_max=200):
    """L(s, f₂₀) via Euler product (Re(s) > k/2+1 = 11 필요)
    ...
    for p in primes:
        ...
        ps = mpmath.power(p, s)
        p2s = ps * ps
        p19 = mpmath.power(p, K_WEIGHT - 1)
        factor = 1 - mpmath.mpf(a_p) / ps + p19 / p2s
        if abs(factor) > 1e-50:
            result /= factor
    return result

def L_dirichlet_sum_w20(s, N_max=N_TERMS):
    """L(s, f₂₀) via Dirichlet sum"""
    result = mpmath.mpc(0)
    for n in range(1, N_max + 1):
        result += mpmath.mpf(A20_INT[n]) / mpmath.power(n, s)
    return result

# Re(s) = 12 (수렴 영역: Re(s) > 11)
log(f"  비교: Re(s)=12 (수렴 영역)")
for sp in [mpmath.mpc(12, 5), mpmath.mpc(12, 10), mpmath.mpc(12, 15)]:
    L_e = L_euler_product_w20(sp)
    L_d = L_dirichlet_sum_w20(sp)
...
if pass_count == 5:
    verdict = "★★★ 강양성 — weight 20에서 ξ-다발 4성질 확립"
elif pass_count == 4:
    verdict = "★★ 조건부 양성"
elif pass_count == 3:
    verdict = "★ 약양성"
...
log()
log(f"  L-함수: Weight 20 cusp form (Δ·E₈), level {N_LEVEL}")
log(f"  Λ(s) = (2π)^{{-s}} Γ(s) L(s,f₂₀), FE: Λ(s) = Λ({K_WEIGHT}-s)")
log(f"  계산: Mellin 분할 ({N_TERMS}항, 지수 수렴)")
log()

# Weight 불변성 비교 (#125 vs #126 vs #127)
if kappa_results:
    log("=" * 72)
    log("Weight 불변성 비교 (3데이터)")
    log("=" * 72)
    log(f"  #125 (w=12, Δ):     κδ² slope = 2.0008 ± 0.0006")
    log(f"  #126 (w=16, Δ·E₄):  κδ² slope = 1.9989 ± 0.0035")
    log(f"  #127 (w=20, Δ·E₈):  κδ² slope = {mean_slope:.4f} ± {std_slope:.4f}")
    all_slopes = [2.0008, 1.9989, mean_slope]
    grand_mean = np.mean(all_slopes)
    grand_std = np.std(all_slopes)
    log(f"  3-weight 평균: {grand_mean:.4f} ± {grand_std:.4f}")
    if grand_std < 0.01 and abs(grand_mean - 2.0) < 0.01:
        log(f"  → Weight 불변성 ★★★ 강화 (w=12,16,20 모두 slope≈2.0)")
    elif abs(mean_slope - 2.0) < 0.05:
        log(f"  → Weight 불변성 ★★ 조건부")
    else:
