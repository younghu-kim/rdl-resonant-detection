...
#### 경계 탐사 원칙 (Boundary Exploration Principle)

**프레임워크가 "작동하는 영역"과 "깨지는 영역"의 경계를 만나면, 멈추지 말고 계속 파고들어라.**

경계에서 파생되는 연구가 가장 가치 있다:
- **경계 발견**: 실험이 실패하거나 조건부 양성일 때 → "왜 여기서 깨지는가?" 질문
- **경계 분류**: 깨지는 이유가 (a) 수치적 한계인가, (b) 구조적 한계인가, (c) 새로운 수학이 필요한 영역인가
- **경계 확장**: 구조적 한계라면 → 한계를 극복하는 새 도구/이론 시도
- **경계 기록**: 모든 경계를 `board/boundaries.md`에 축적. 각 경계는 잠재적 후속 논문 씨앗

예시:
- GL(2) σ-유일성 FAIL → Γ-인자 메커니즘 발견 → 별도 소절
- Epstein zeta 조건부 양성 → 함수방정식 없는 L-함수의 경계 → 후속 논문 방향
- GL(3)에서 새로운 실패 모드 → 새 추측 또는 새 도구

**한 논문이 끝나면 "경계 목록"에서 다음 논문 주제를 고른다.**
논문 완결 선언 전에 반드시 경계 목록을 갱신하고, 어떤 경계가 다음 논문으로 갈지 명시한다.

#### 판단 결과 유형
- **후속 실험**: 이전 결과의 논리적 다음 단계
- **경계 탐사**: 실패/조건부 양성에서 경계를 파고드는 실험 ← **최우선**
- **재현 실험**: 예상 밖 결과 → 재현으로 확인
- **대기열 실험**: 급한 후속이 없을 때 미착수 과제에서 선택
- **증명 시도**: 수치적으로 확립된 결과를 엄밀한 정리로 승격 (Phase 2.5)
- **논문 구조 개선**: 실험은 충분하고 서술/구조를 개선할 때
- **문헌 조사**: 관련 논문 검색으로 새 아이디어 탐색
- **휴식**: 실험이 돌아가고 있고, 새 결과도 없으면 아무것도 안 함

### Phase 2.5: 수학 증명 탐색 — 이론가 모드

**매 사이클마다 실험 전에 반드시 이 단계를 거친다.** 수치 실험만 반복하면 논문은 "관측 모음집"에 그친다. 수학 저널에서 가치를 인정받으려면 **정리(theorem)와 증명(proof)**이 있어야 한다.

#### 2.5-1. 증명 가능성 스캔

매 사이클 시작 시 다음을 점검:

1. **현재 확립된 관측(Observation) 중 정리로 승격 가능한 것이 있는가?**
   - 편각 원리(argument principle)의 직접 귀결인가?
   - 로랑 전개(Laurent expansion)에서 따르는가?
   - 함수방정식(functional equation)의 재서술인가?
   - 아다마르 곱(Hadamard product)의 성질인가?
   - 스톡스 정리(Stokes' theorem) / 가우스-보네의 응용인가?
   - 위상적 불변량(winding number, Chern class)의 정수성인가?

2. **기존 정리(Part II-b)를 강화할 수 있는가?**
   - 더 약한 가정에서 같은 결론?
   - 더 강한 결론 (예: 수렴 속도, 오차 한계)?
   - 새로운 따름정리(corollary)?

3. **σ-국소화 (FWHM < 0.004) 증명의 실마리가 있는가?**
   - 이것이 증명되면 RH의 새 formulation이 됨
   - 매 사이클마다 "왜 곡률 플럭스가 σ=1/2에 집중되는가?"를 1분이라도 생각할 것

#### 2.5-2. 증명 시도 절차

증명 가능한 후보가 발견되면:

1. **서술 (Statement)**: 정확한 조건과 결론을 Definition-Theorem 형식으로 작성
2. **증명 전략 (Strategy)**: 사용할 도구 명시 (편각 원리, CR 방정식, 잔류값 정리 등)
3. **증명 작성 (Proof)**: LaTeX로 완전한 증명 작성
4. **자기 검증**: 증명의 각 단계가 논리적으로 따르는가?
   - 가정은 모두 사용되었는가?
   - 결론은 가정으로부터 따르는가?
   - 빠진 경우(edge case)는 없는가?
5. **비평가 검증**: Phase 1.5와 동일한 Red Team 적용
6. **논문 삽입**: Part II-b (Theoretical Foundations)에 추가

#### 2.5-3. 증명 대상 우선순위

| 우선순위 | 대상 | 상태 | 증명 전략 |
|---------|------|------|----------|
| **1 (최우선)** | σ-국소화의 이론적 근거 | 미증명 (수치만) | 미지 — 이것이 핵심 열린 문제 |
| **2** | 중간점 곡률의 비국소성 | 관측 | 아다마르 곱 NN 항 소거 + 잔여항 분석 |
| **3** | κ 차선도 구조 log(t/2π) | 관측 | Stirling + ψ(s/2) 점근 전개 |
| **4** | GUE 간격과 Δκ 상관 | 관측 | Montgomery pair correlation + κ 로랑 |
| **5** | Dirichlet 보편성 정량화 | 관측 | conductor 의존성의 해석적 표현 |

#### 2.5-4. 증명 실패 시

증명에 실패하면:
1. **실패 유형 기록**: 어디서 막혔는가? (기술적 한계? 더 강한 가정 필요? 근본적 장벽?)
2. **부분 결과 보존**: 증명 가능한 약한 버전이 있으면 그것이라도 정리로 기록
3. **수치 실험으로 전환**: 증명 전략에서 실마리를 얻어 검증 실험 설계
4. **반성문**: scripts/memory/episodic/proof_attempt_{날짜}.md 에 기록

#### 2.5-5. 기존 정리 참조

현재 Part II-b에 증명된 정리들:

| 정리 | 내용 | 근거 |
|------|------|------|
| Thm (홀로노미) | mono = 2πm at order-m zero | Argument principle |
| Thm (곡률 집중) | κ ~ m²/\|s-ρ\|² | Laurent expansion of Λ'/Λ |
| Thm (가우스-보네) | ∫∫F₂ = 2πN | Stokes + argument principle |
| Thm (보편성) | Thm 1-3은 GL(n) L-함수 전체에 성립 | 증명이 함수 구체적 형태에 무관 |
| Cor (이중기준) | mono+κ → 완벽 TP/FP 분리 | Thm 1+2의 귀결 |

**이 정리들의 가정을 약화하거나, 결론을 강화하거나, 새 따름정리를 발견하는 것도 유효한 기여이다.**
...
- [ ] **σ-프로파일 FWHM 윈도우 아티팩트**: FWHM은 탐색 윈도우 크기에 민감. σ₀±0.6(81점)과 σ₀±0.5(61점)에서 동일 영점의 FWHM이 18× 차이. 표준: σ₀±0.3, ≥101점 (간격≤0.006). 사이클 #43 DH FWHM 41× 주장 기각 사례.

- [ ] **EC 계수 정확성**: 타원곡선 계수(a₁..a₆)는 반드시 LMFDB API 직접 확인. 임의 작성 금지. C-349에서 12곡선 계수 전부 부정확했던 사례.
- [ ] **Confound 분석 필수**: 두 변수(X,Y) 상관 주장 시 제3 변수(Z) 교란 점검. Partial correlation(X vs Y | Z) 계산 필수. C-349에서 rank vs A(t₀) 겉보기 양성→conductor 제어 시 음성.
