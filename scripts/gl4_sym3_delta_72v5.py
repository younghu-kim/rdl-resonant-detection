#!/usr/bin/env python3
"""
[Project RDL] #72v5 — GL(4) sym³(Δ) 감마 인자 수정 (μ 교정)
=============================================================================
#72v1~v4 실패 근본 원인:
  μ=(5.5,6.5,16.5,17.5)는 L(s,Δ×Δ) Rankin-Selberg의 감마 인자였음!
  sym³(Δ)의 정확한 감마 인자: μ=(5.5, 6.5, 16.5, 17.5)

  도출: sym³(Δ)의 Hodge type (33,0),(22,11),(11,22),(0,33)
    → Γ_C(s+11/2)·Γ_C(s+33/2)  (Γ_C(s)=2(2π)^{-s}Γ(s))
    → Γ_ℝ(s+11/2)·Γ_ℝ(s+11/2+1)·Γ_ℝ(s+33/2)·Γ_ℝ(s+33/2+1)
    → μ = (11/2, 13/2, 33/2, 35/2) = (5.5, 6.5, 16.5, 17.5)

  계수 계산(e1, e2, 재귀식)은 수학적으로 정확 — 변경 불필요.
  FE 검증점: Re(s)=2~2.5 유지 (v4에서 확인된 backward 수렴 조건).

L-함수: L(s, sym³Δ), degree=4, conductor N=1
  감마: μ=(5.5, 6.5, 16.5, 17.5) — Γ_C(s+11/2)·Γ_C(s+33/2)
  Λ(s) = π^{-2s}·Γ((s+5.5)/2)·Γ((s+6.5)/2)·Γ((s+16.5)/2)·Γ((s+17.5)/2)·L(s)
...
OUTFILE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "..", "results", "gl4_sym3_delta_72v5.txt")
...
MU = [5.5, 6.5, 16.5, 17.5]  # ★ 핵심 수정: sym³(Δ) 정확한 감마 인자
...
if nt_pass >= 3:
    log(f"  ✅ 비자명 FE 통과! — 계수 + 감마 인자 μ=(5.5,6.5,16.5,17.5) 정합 확인")
elif nt_pass >= 1:
...
if nt_pass >= 3:
    log(f"       ✅ PASS — 계수+감마 μ=(5.5,6.5,16.5,17.5) 정합 확인")
else:
    log(f"       ❌ FAIL")
