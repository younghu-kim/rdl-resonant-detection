...
"""
=============================================================================
[RDL 실험 #204] GL(4) sym³(37a1) — 독립 degree-4 κδ² log-log slope 검증
=============================================================================
배경:
  #203: GL(4) sym³(Δ) ★★★ (slope=2.0000±0.0001, 4/5 PASS)
  비평가 지적: 11a1/Δ 편향 — sym²(11a1), sym³(Δ) 모두 11a1 파생.
  이번 실험: 완전 독립 타원곡선 37a1 기반 sym³.

대상: sym³(37a1), conductor N=50653, degree 4
  - 37a1: y² + y = x³ - x (Cremona label), rank 1
  - PARI lfunsympow(E, 3) 직접 사용 (검증됨)
  - gammaV = [-1, 0, 0, 1], k=4
  - center = 2.0 (arithmetic normalisation, critical line Re(s)=k/2=2)

방법론: SC3a κδ² + SC3b 모노드로미 + SC3c σ-유일성
  동일 프로토콜: δ=[0.001..0.03], log|κδ²-1| vs log(δ), slope=2.0 이론값

목적: 11a1 편향 해소. 성공 시 11행 비교표, "degree-4 보편성" 강화.

결과: results/gl4_sym3_37a1_kd2_204.txt
=============================================================================
"""
...
RESULT_FILE = os.path.join(RESULT_DIR, 'gl4_sym3_37a1_kd2_204.txt')
...
log("=" * 72)
log("[실험 #204] GL(4) sym³(37a1) — 독립 degree-4 κδ² log-log slope 검증")
log("=" * 72)
log(f"시작: {time.strftime('%Y-%m-%d %H:%M:%S')}")
log(f"L-함수: sym³(37a1), N=50653, degree 4")
log(f"center = 2.0 (arithmetic normalisation, k=4)")
log(f"gammaV = [-1, 0, 0, 1] (PARI lfunsympow)")
log(f"δ 범위: [0.001, 0.002, 0.003, 0.005, 0.008, 0.01, 0.02, 0.03]")
log(f"방법: PARI lfunsympow(E,3) + lfun 직접 평가")
log()

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 파라미터
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CENTER = 2.0  # arithmetic normalisation: critical line at Re(s) = k/2 = 2
DELTAS = [0.001, 0.002, 0.003, 0.005, 0.008, 0.01, 0.02, 0.03]
T_SCAN_MAX = 20.0
H_DERIV = 1e-8  # 수치 미분 간격

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 1. PARI 초기화 + sym³(37a1) L-함수 생성
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

log(f"{T()} [1/7] PARI 초기화 + sym³(37a1)...")
...
# 37a1 타원곡선 생성
gp('E = ellinit([0, 1, 1, -23, -50])')  # 37a1 Cremona model
cond = int(gp('ellglobalred(E)[1]'))
log(f"  E = 37a1: y² + y = x³ - x, conductor={cond}")
assert cond == 37, f"conductor mismatch: {cond}"

# sym³ L-함수
gp('global(L3); L3 = lfunsympow(E, 3)')
log(f"  L3 = lfunsympow(E, 3)")

# 기본 검증
gamma_v = str(gp('L3[3]'))
n_sym3 = int(gp('L3[5]'))
log(f"  gammaV = {gamma_v}")
log(f"  conductor = {n_sym3}")

# FE 검증
fe_score = float(gp("lfuncheckfeq(L3)"))
log(f"  lfuncheckfeq = {fe_score:.1f}  {'✅' if fe_score <= -8 else '⚠️'}")

# L(center) 확인 — 산술 정규화에서 center=2
try:
    L_center_re = float(gp("real(lfun(L3, 2))"))
    L_center_im = float(gp("imag(lfun(L3, 2))"))
    L_center_val = complex(L_center_re, L_center_im)
    log(f"  L(center=2) = {L_center_val.real:.6e} + {L_center_val.imag:.6e}i")
    has_central_zero = abs(L_center_val) < 1e-10
    if has_central_zero:
        log(f"  → L(2) ≈ 0: 중앙 영점 존재")
    else:
        log(f"  → L(2) ≠ 0")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 2. 영점 탐색 (lfunzeros 사용 — lfunsympow는 안정)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

log(f"{T()} [2/7] 영점 탐색 (lfunzeros, t∈[0, {T_SCAN_MAX}])...")
t0_s = time.time()

zeros_raw = gp(f"lfunzeros(L3, {T_SCAN_MAX})")
...
def eval_L_complex(s_re, s_im):
    """L(s) 복소수 값 반환"""
    try:
        v = complex(gp(f"lfun(L3, {s_re:.15f} + I*{s_im:.15f})"))
        return v
    except Exception as e:
        return None
...
def conn_gamma(s_re, s_im):
    ...
    for mu in GAMMA_V:
        ...
        try:
            ...
            return None
    # (1/2)log(N) + (1/2)Σψ - (d/2)·log(π)  [d=4]
    conn_re = 0.5 * math.log(N_COND) + 0.5 * total_re - 2.0 * math.log(math.pi)
...
log(f"  conn(s) = (1/2)log(N) + (1/2)Σψ((s+μ_j)/2) - 2·log(π), N=50653, μ∈[-1,0,0,1]")
...
log("종합 판정")
log("=" * 72)
log(f"  L-함수: sym³(37a1), degree 4, N=50653")
log(f"  center: {CENTER}")
log(f"  gammaV: [-1, 0, 0, 1]")
