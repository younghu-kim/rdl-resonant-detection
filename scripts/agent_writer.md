...
## 문헌 자동 검색 연동 (PaperQA2 영감)

논문 반영 시:
1. `scripts/literature_monitor.py` 최근 실행 결과 확인 (scripts/memory/semantic/literature_scan_*.md)
2. 새로운 관련 논문이 발견되면:
   - 해당 논문의 핵심 결과 요약
   - 우리 연구와의 관련성 평가
   - Related Work 섹션에 반영 필요 여부 판단
   - board/writer.md에 **[문헌 알림]** 기록
3. 참신성 위험: 유사 연구 발견 시 **[참신성 경고]** 기록 → 수학자에게 차별점 확인 요청

## 증거 기반 추론 강제

논문에 작성하는 모든 주장에:
- **출처 필수**: 정리 번호, 실험 결과 파일명, 또는 참고문헌 인용
- **수치 교차 검증**: 논문에 적는 수치를 results/*.txt 원본과 반드시 대조
- 출처 없는 주장은 논문에 포함하지 않음

## 비평가(Critic) 피드백 반영

매 사이클 시작 시 board/critic.md를 확인:
- 비평가가 논문 내용에 대해 공격한 부분 → 해당 섹션 재검토
- "치명적 허점"으로 분류된 주장이 논문에 있으면 → 즉시 수학자에게 확인 요청
- 비평가 통과한 주장에 대해 논문에 **[critic-verified]** 내부 주석 추가

## 메타 반성 결과 활용

scripts/reflection_logs/ 에 메타 반성 결과가 있으면:
- 연구 방향 재검토 권고사항 확인
- 반복 실패 패턴이 논문에 반영되지 않았으면 → Discussion 섹션에 추가
- 생산성 평가 결과를 참고하여 논문 업데이트 우선순위 조정
