...
# 이분법으로 영점 정밀화
mpmath.mp.dps = 40  # 정밀화에서 높은 정밀도
zeros_list = []
for idx in sc:
    a, b = float(t_grid[idx]), float(t_grid[idx + 1])
    try:
        root = float(mpmath.findroot(mpmath.siegelz, mpmath.mpf(a + b) / 2))
        if a - 0.1 <= root <= b + 0.1:
            zeros_list.append(root)
    except Exception:
        # 이분법 대체
        fa = float(mpmath.siegelz(a))
        for _ in range(60):
            mid = (a + b) / 2
            fm = float(mpmath.siegelz(mid))
            if fa * fm < 0:
                b = mid
            else:
                a, fa = mid, fm
        zeros_list.append((a + b) / 2)

mpmath.mp.dps = 30
zeros = np.array(sorted(set(round(z, 10) for z in zeros_list if 1.0 < z <= T_MAX)))
N = len(zeros)
